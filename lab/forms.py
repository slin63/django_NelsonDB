...
class LogSeedDataOnlineForm(forms.Form):
	...
	stock__stock_status = forms.CharField(widget=forms.TextInput(attrs={'placeholder':'Stock Status'}), help_text="Stock Date", required=False)
	stock__stock_date = forms.CharField(widget=forms.TextInput(attrs={'placeholder':'Stock Date'}), help_text="Stock Status", required=False)
	...
	stock__passport__collecting__user = forms.ModelChoiceField(queryset=User.objects.all(), empty_label="--- Collected By ---", help_text="Collector", required=False)
