...
def file_storage(request):
    ...
    os_path = path.dirname(path.realpath(__file__)) + '/../media/files/'
    sub_directories = listdir(os_path)
    file_dict = {}
    for loc in sub_directories:
        file_dict[loc] = FileDump.objects.filter(file_subdirectory=loc)
    all_files = FileDump.objects.all()

    context_dict = {'file_form': file_form, 'saved': saved, 'all_files': all_files, 'logged_in_user': request.user, 'sub_directories': sub_directories, 'file_dict': file_dict
    }
    return render_to_response('lab/file_storage/file_storage.html', context_dict, context)
    ...
    if request.method == 'POST':
        ...
        if user:
            if user.is_active:
                ...
                return render_to_response('lab/login.html', {'next_url': next_url, 'disabled_account': 'disabled'},
                                          context)
        else:
...
def profile_change_password(request):
    ...
    if request.method == 'POST':
        ...
        if change_password_form.is_valid():
            if change_password_form.cleaned_data['new_password'] == change_password_form.cleaned_data[
                'new_password_repeat']:
...
def experiment(request, experiment_name_url):
    ...
    if experiment_name is not 'search':
        try:
            ...
            obs_type_list = ['stock', 'isolatestock', 'isolate', 'maize', 'plot', 'plant', 'sample', 'environment',
                             'dna', 'tissue', 'plate', 'well', 'microbe', 'culture', 'extract', 'maize']
            for obs_type in obs_type_list:
                obs_data = "%s_data" % (obs_type)
                try:
                    obs_type_data = ObsTracker.objects.filter(experiment__name=experiment_name,
                                                              obs_entity_type=obs_type)
...
def experiment_edit(request, experiment_id):
    ...
    if request.method == 'POST':
        ...
        experiment_info = Experiment.objects.filter(id=experiment_id).values('name', 'field', 'user', 'start_date',
                                                                             'purpose', 'comments')
...
def show_all_seedinv_taxonomy(request):
    ...
    if request.session.get('checkbox_pedigree', None):
        checkbox_pedigree_list = request.session.get('checkbox_pedigree')
        for pedigree in checkbox_pedigree_list:
            taxonomy = Stock.objects.filter(pedigree=pedigree).values('pedigree',
...
def suggest_taxonomy(request):
    ...
    if starts_with:
        if request.session.get('checkbox_pedigree', None):
            ...
            taxonomy_list = list(Taxonomy.objects.filter(population__contains=starts_with, common_name='Maize').values(
                'population').distinct())
            for t in taxonomy_list:
                t['input'] = '<input type="checkbox" name="checkbox_taxonomy" value="%s">' % (t['population'])
                t['passport__taxonomy__population'] = t['population']
...
def update_seed_info(request, stock_id):
    ...
    if request.method == 'POST':
            ...
            with transaction.atomic():
                try:
                    ...
                    if obs_tracker_stock_form.cleaned_data['obs_plot__plot_id'] != '':
                        obs_tracker.obs_plot = ObsPlot.objects.get(
                            plot_id=obs_tracker_stock_form.cleaned_data['obs_plot__plot_id'])
                    else:
                    ...
                    updated_taxonomy, created = Taxonomy.objects.get_or_create(
                        binomial=obs_tracker_stock_form.cleaned_data['stock__passport__taxonomy__binomial'],
        ...
        stock_data = ObsTracker.objects.filter(obs_entity_type='stock', stock_id=stock_id).values('experiment',
                                                                                                  ...
                                                                                                  'stock__passport__taxonomy__binomial',
...
def stock_page_measurement_plot(request):
    ...
    for exp in experiments:
        m = Measurement.objects.filter(obs_tracker__experiment_id=exp.experiment_id,
                                       measurement_parameter_id=parameter_id)
    ...
    for ms in measurements:
        ...
        try:
            ...
            try:
                ...
                value = ms.value
        data.append({'parameter': "%s: %s" % (ms.obs_tracker.experiment.name, ms.measurement_parameter.parameter),
                     'id': "%s: %s" % (ms.obs_tracker.obs_id, ms.id), 'value': value})
    ...
    for ms in measurements:
        ...
        try:
            ...
            try:
                ...
                value = ms.value
        data.append({'parameter': "%s: %s" % (ms.obs_tracker.stock.seed_id, ms.measurement_parameter.parameter),
                     'id': "%s: %s" % (ms.obs_tracker.obs_id, ms.id), 'value': value})
...
def update_isolate_info(request, isolate_id):
    ...
    if request.method == 'POST':
        ...
        isolate_data = Isolate.objects.filter(id=isolate_id).values('isolate_id', 'location', 'locality', 'stock_date',
                                                                    'extract_color', 'organism', 'comments')
        try:
...
def edit_info(request, obj_type, obj_id):
    ...
    if obj_type == 'measurement_parameter':
        if request.method == 'POST':
            ...
            parameter_data = MeasurementParameter.objects.filter(id=obj_id).values('parameter', 'parameter_type',
                                                                                   'protocol', 'trait_id_buckler',
        ...
        if request.method == 'POST':
            ...
            medium_data = Medium.objects.filter(id=obj_id).values('citation', 'media_name', 'media_type',
                                                                  'media_description', 'media_preparation', 'comments')
        ...
        if request.method == 'POST':
            ...
            location_data = Location.objects.filter(id=obj_id).values('locality', 'location_name', 'building_name',
                                                                      'room', 'shelf', 'column', 'box_name', 'comments')
        ...
        if request.method == 'POST':
            ...
            taxonomy_data = Taxonomy.objects.filter(id=obj_id).values('binomial', 'population', 'alias', 'race',
                                                                      'subtaxa')
...
def download_stock_used_experiment(request, experiment_name):
    ...
    try:
        stock_associated_experiment = ObsTracker.objects.filter(experiment__name=experiment_name,
                                                                obs_entity_type='stock')
    except ObsTracker.DoesNotExist:
        stock_associated_experiment = None
    try:
        collected_stock_data = ObsTrackerSource.objects.filter(source_obs__experiment__name=experiment_name,
                                                               target_obs__obs_entity_type='stock').values_list(
            'target_obs__stock__id', flat=True)
    except ObsTracker.DoesNotExist:
...
def download_row_experiment(request, experiment_name):
    ...
    plot_loader = find_row_from_experiment(experiment_name)
    writer = csv.writer(response)
    writer.writerow(['Plot ID', 'Plot Name', 'Field', 'Source Stock', 'Range', 'Plot', 'Block', 'Rep', 'Kernel Num',
    ...
    for row in plot_loader:
        writer.writerow([row.obs_plot.plot_id, row.obs_plot.plot_name, row.field.field_name, row.stock.seed_id,
                         row.obs_plot.range_num, row.obs_plot.plot, row.obs_plot.block, row.obs_plot.rep,
                         row.obs_plot.kernel_num, row.obs_plot.planting_date, row.obs_plot.harvest_date,
                         row.obs_plot.comments])
    return response
...
def passport(request, passport_id):
    ...
    for obs_type in obs_type_list:
        ...
        try:
            collecting_obs_type = ObsTracker.objects.get(obs_entity_type='stock', stock__passport=passport).exclude(
                obs_data_type='1')
        except ObsTracker.DoesNotExist:
            ...
            for disease_id in checkbox_disease_list:
                for taxonomy_id in checkbox_taxonomy_list:
                    isolatestocks = IsolateStock.objects.filter(disease_info__id=disease_id,
                                                                passport__taxonomy__id=taxonomy_id)
...
def show_all_isolatestock_disease(request):
    ...
    if request.session.get('checkbox_isolatestock_taxonomy', None):
        ...
        for taxonomy_id in checkbox_isolatestock_taxonomy:
            disease = IsolateStock.objects.filter(passport__taxonomy__id=taxonomy_id).values('disease_info__id',
                                                                                             'disease_info__common_name',
                                                                                             'passport__taxonomy__binomial').distinct()
            isolatestock_disease_list = list(chain(disease, isolatestock_disease_list))
        for p in isolatestock_disease_list:
            p['input'] = '<input type="checkbox" name="checkbox_isolatestock_disease_id" value="%s">' % (
            p['disease_info__id'])
    else:
    ...
    if starts_with:
        if request.session.get('checkbox_isolatestock_disease', None):
            ...
            for disease_id in checkbox_isolatestock_disease:
                taxonomy = IsolateStock.objects.filter(disease_info__id=disease_id,
                                                       passport__taxonomy__binomial__contains=starts_with).values(
                    'passport__taxonomy__id', 'disease_info__common_name', 'passport__taxonomy__binomial',
                    'passport__taxonomy__alias', 'passport__taxonomy__race', 'passport__taxonomy__subtaxa').distinct()
                isolatestock_taxonomy_list = list(chain(taxonomy, isolatestock_taxonomy_list))
            for t in isolatestock_taxonomy_list:
                t['input'] = '<input type="checkbox" name="checkbox_isolatestock_taxonomy_id" value="%s">' % (
                t['passport__taxonomy__id'])
        else:
            isolatestock_taxonomy_list = list(
                Taxonomy.objects.filter(binomial__contains=starts_with, common_name='IsolateStock').values('id',
                                                                                                           'binomial',
                                                                                                           'alias',
                                                                                                           'race',
                                                                                                           'subtaxa').distinct())
...
def suggest_isolatestock_disease(request):
    ...
    if starts_with:
        if request.session.get('checkbox_isolatestock_taxonomy', None):
            checkbox_isolatestock_taxonomy = request.session.get('checkbox_isolatestock_taxonomy')
            for taxonomy_id in checkbox_isolatestock_taxonomy:
                disease = IsolateStock.objects.filter(disease_info__common_name__contains=starts_with,
                                                      passport__taxonomy__id=taxonomy_id).values('disease_info__id',
                                                                                                 'disease_info__common_name',
                                                                                                 'passport__taxonomy__binomial').distinct()
                isolatestock_disease_list = list(chain(disease, isolatestock_disease_list))
            for t in isolatestock_disease_list:
                t['input'] = '<input type="checkbox" name="checkbox_isolatestock_disease_id" value="%s">' % (
                t['disease_info__id'])
        else:
            isolatestock_disease_list = list(
                DiseaseInfo.objects.filter(common_name__contains=starts_with).values('id', 'common_name').distinct())
            for t in isolatestock_disease_list:
                t['input'] = '<input type="checkbox" name="checkbox_isolatestock_disease_id" value="%s">' % (t['id'])
                t['disease_info__common_name'] = t['common_name']
                t['passport__taxonomy__binomial'] = ''
    ...
    if request.method == 'POST':
        ...
        if new_experiment_form.is_valid():
            ...
            try:
                ...
                experiment = Experiment.objects.create(user=new_experiment_user, field=new_experiment_field,
                                                       name=new_experiment_name, start_date=new_experiment_start_date,
                                                       purpose=new_experiment_purpose, comments=new_experiment_comments)
                experiment_added = True
        else:
            print(new_experiment_form.errors)
            name_check_fail = False
...
def suggest_maize_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        maize_experiment_list = ObsTracker.objects.filter(obs_entity_type='maize',
...
def show_all_maize_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    maize_experiment_list = ObsTracker.objects.filter(obs_entity_type='maize').values('experiment__name',
...
def download_maize_experiment(request, experiment_name):
    ...
    maize_data = find_maize_from_experiment(experiment_name)
    writer = csv.writer(response)
    writer.writerow(['Maize ID', 'County', 'Sub Location', 'Village', 'Weight', 'Harvest Date', 'Storage Months',
                     'Storage Conditions', 'Maize Variety', 'Seed Source', 'Moisture Content', 'Source Type',
                     'Appearance', 'GPS Latitude', 'GPS Longitude', 'GPS Altitude', 'GPS Accuracy', 'Photo Filename'])
    for row in maize_data:
        writer.writerow([row.maize_sample.maize_id, row.maize_sample.county, row.maize_sample.sub_location,
                         row.maize_sample.village, row.maize_sample.weight, row.maize_sample.harvest_date,
                         row.maize_sample.storage_months, row.maize_sample.storage_conditions,
                         row.maize_sample.maize_variety, row.maize_sample.seed_source,
                         row.maize_sample.moisture_content, row.maize_sample.source_type, row.maize_sample.appearance,
                         row.maize_sample.gps_latitude, row.maize_sample.gps_longitude, row.maize_sample.gps_altitude,
                         row.maize_sample.gps_accuracy, row.maize_sample.photo])
    return response
...
def download_maize_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp Name', 'Maize ID', 'County', 'Sub Location', 'Village', 'Weight', 'Harvest Date', 'Storage Months',
         'Storage Conditions', 'Maize Variety', 'Seed Source', 'Moisture Content', 'Source Type', 'Appearance',
         'GPS Latitude', 'GPS Longitude', 'GPS Altitude', 'GPS Accuracy', 'Photo Filename'])
    for row in maize_data:
        writer.writerow(
            [row.experiment.name, row.maize_sample.maize_id, row.maize_sample.county, row.maize_sample.sub_location,
             row.maize_sample.village, row.maize_sample.weight, row.maize_sample.harvest_date,
             row.maize_sample.storage_months, row.maize_sample.storage_conditions, row.maize_sample.maize_variety,
             row.maize_sample.seed_source, row.maize_sample.moisture_content, row.maize_sample.source_type,
             row.maize_sample.appearance, row.maize_sample.gps_latitude, row.maize_sample.gps_longitude,
...
def download_isolates_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Isolate ID', 'Stock Date', 'Extract Color', 'Organism', 'Location Name', 'Source Seed ID', 'Source Plot ID',
         'Source Culture ID', 'Source IsolateStock ID', 'Comments'])
    for row in isolate_data:
        writer.writerow(
            [row.isolate.isolate_id, row.isolate.stock_date, row.isolate.extract_color, row.isolate.organism,
             row.location, row.stock.seed_id, row.obs_plot.plot_id, row.obs_culture.culture_id,
             row.isolatestock.isolatestock_id, row.isolate.comments])
    return response
...
def download_microbe_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'Microbe ID', 'Microbe Type', 'Comments', 'Source Plot ID', 'Source Seed ID', 'Source Plant ID',
         'Source Tissue ID', 'Source Culture ID'])
    for row in microbe_data:
        writer.writerow(
            [row.experiment.name, row.obs_microbe.microbe_id, row.obs_microbe.microbe_type, row.obs_microbe.comments,
             row.obs_plot.plot_id, row.stock.seed_id, row.obs_plant.plant_id, row.obs_tissue.tissue_id,
             row.obs_culture.culture_id])
    return response
...
def suggest_microbe_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        microbe_experiment_list = ObsTracker.objects.filter(obs_entity_type='microbe',
                                                            experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                                  :2000]
    else:
...
def show_all_microbe_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    microbe_experiment_list = ObsTracker.objects.filter(obs_entity_type='microbe').values('experiment__name',
                                                                                          'experiment__field__field_name',
                                                                                          'experiment__field__id',
                                                                                          'experiment__id').distinct()[
                              :2000]
...
def download_microbe_experiment(request, experiment_name):
    ...
    microbe_data = find_microbe_from_experiment(experiment_name)
    writer = csv.writer(response)
    writer.writerow(['Microbe ID', 'Microbe Type', 'Comments', 'Source Plot ID', 'Source Seed ID', 'Source Plant ID',
                     'Source Tissue ID', 'Source Culture ID'])
    for row in microbe_data:
        writer.writerow(
            [row.obs_microbe.microbe_id, row.obs_microbe.microbe_type, row.obs_microbe.comments, row.obs_plot.plot_id,
             row.stock.seed_id, row.obs_plant.plant_id, row.obs_tissue.tissue_id, row.obs_culture.culture_id])
    return response
...
def download_env_data(request):
    ...
    for row in env_data:
        writer.writerow([row.experiment.name, row.obs_env.environment_id, row.field.field_name, row.obs_env.longitude,
                         row.obs_env.latitude, row.obs_env.comments])
    return response
...
def suggest_env_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        env_experiment_list = ObsTracker.objects.filter(obs_entity_type='environment',
                                                        experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                              :2000]
    else:
...
def show_all_env_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    env_experiment_list = ObsTracker.objects.filter(obs_entity_type='environment').values('experiment__name',
                                                                                          'experiment__field__field_name',
                                                                                          'experiment__field__id',
                                                                                          'experiment__id').distinct()[
                          :2000]
...
def download_env_experiment(request, experiment_name):
    ...
    for row in env_data:
        writer.writerow([row.obs_env.environment_id, row.field.field_name, row.obs_env.longitude, row.obs_env.latitude,
                         row.obs_env.comments])
    return response
...
def suggest_row_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        row_experiment_list = ObsTracker.objects.filter(obs_entity_type='row',
                                                        experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                              :2000]
    else:
...
def show_all_row_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    row_experiment_list = ObsTracker.objects.filter(obs_entity_type='row').values('experiment__name',
                                                                                  'experiment__field__field_name',
                                                                                  'experiment__field__id',
...
def download_sample_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'Sample ID', 'Sample Type', 'Sample Name', 'Source Seed ID', 'Source Plot ID', 'Source Plant ID',
         'Weight', 'Volume', 'Density', 'Num Kernels', 'Photo', 'Comments'])
    for row in sample_data:
        writer.writerow(
            [row.experiment, row.obs_sample.sample_id, row.obs_sample.sample_type, row.obs_sample.sample_name,
             row.stock.seed_id, row.obs_plot.plot_id, row.obs_plant.plant_id, row.obs_sample.weight,
             row.obs_sample.volume, row.obs_sample.density, row.obs_sample.kernel_num, row.obs_sample.photo,
             row.obs_sample.comments])
    return response
...
def suggest_sample_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        sample_experiment_list = ObsTracker.objects.filter(obs_entity_type='sample',
                                                           experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                                 :2000]
    else:
...
def show_all_sample_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    sample_experiment_list = ObsTracker.objects.filter(obs_entity_type='sample').values('experiment__name',
                                                                                        'experiment__field__field_name',
                                                                                        'experiment__field__id',
                                                                                        'experiment__id').distinct()[
                             :2000]
...
def download_sample_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Sample ID', 'Sample Type', 'Sample Name', 'Source Seed ID', 'Source Plot ID', 'Source Plant ID', 'Weight',
         'Volume', 'Density', 'Num Kernels', 'Photo', 'Comments'])
    for row in sample_data:
        writer.writerow(
            [row.obs_sample.sample_id, row.obs_sample.sample_type, row.obs_sample.sample_name, row.stock.seed_id,
             row.obs_plot.plot_id, row.obs_plant.plant_id, row.obs_sample.weight, row.obs_sample.volume,
             row.obs_sample.density, row.obs_sample.kernel_num, row.obs_sample.photo, row.obs_sample.comments])
    return response
...
def update_sample_info(request, obs_sample_id):
    ...
    if request.method == 'POST':
        ...
        if obs_tracker_samples_form.is_valid():
            with transaction.atomic():
                try:
                    obs_tracker = ObsTracker.objects.get(obs_entity_type='sample', obs_sample_id=obs_sample_id,
                                                         experiment=obs_tracker_samples_form.cleaned_data['experiment'])
                    obs_tracker.isolate_id = 1
                    obs_tracker.maize_sample_id = 1
                    obs_tracker.obs_extract_id = 1
                    if obs_tracker_samples_form.cleaned_data['stock__seed_id'] != '':
                        obs_tracker.stock = Stock.objects.get(
                            seed_id=obs_tracker_samples_form.cleaned_data['stock__seed_id'])
                        try:
                            obs_tracker_source_stock = ObsTrackerSource.objects.get(target_obs=obs_tracker,
                                                                                    source_obs__obs_entity_type='stock',
                                                                                    relationship='sample_from_stock')
                            obs_tracker_source_stock.source_obs = ObsTracker.objects.get(obs_entity_type='stock',
                                                                                         stock__seed_id=
                                                                                         obs_tracker_samples_form.cleaned_data[
                                                                                             'stock__seed_id'])
                            obs_tracker_source_stock.save()
                        except ObsTrackerSource.DoesNotExist:
                            new_source_stock = ObsTrackerSource.objects.create(
                                source_obs=ObsTracker.objects.get(obs_entity_type='stock',
                                                                  stock__seed_id=obs_tracker_samples_form.cleaned_data[
                                                                      'stock__seed_id']), target_obs=obs_tracker,
                                relationship="sample_from_stock")
                    else:
                        obs_tracker.stock = Stock.objects.get(id=1)
                    if obs_tracker_samples_form.cleaned_data['obs_plot__plot_id'] != '':
                        obs_tracker.obs_plot = ObsPlot.objects.get(
                            plot_id=obs_tracker_samples_form.cleaned_data['obs_plot__plot_id'])
                        try:
                            obs_tracker_source_plot = ObsTrackerSource.objects.get(target_obs=obs_tracker,
                                                                                   source_obs__obs_entity_type='plot',
                                                                                   relationship='sample_from_plot')
                            obs_tracker_source_plot.source_obs = ObsTracker.objects.get(obs_entity_type='plot',
                                                                                        obs_plot__plot_id=
                                                                                        obs_tracker_samples_form.cleaned_data[
                                                                                            'obs_plot__plot_id'])
                            obs_tracker_source_plot.save()
                        except ObsTrackerSource.DoesNotExist:
                            new_source_plot = ObsTrackerSource.objects.create(
                                source_obs=ObsTracker.objects.get(obs_entity_type='plot', obs_plot__plot_id=
                                obs_tracker_samples_form.cleaned_data['obs_plot__plot_id']), target_obs=obs_tracker,
                                relationship="sample_from_plot")
                    else:
                        obs_tracker.obs_plot = ObsPlot.objects.get(id=1)
                    if obs_tracker_samples_form.cleaned_data['obs_plant__plant_id'] != '':
                        obs_tracker.obs_plant = ObsPlant.objects.get(
                            plant_id=obs_tracker_samples_form.cleaned_data['obs_plant__plant_id'])
                        try:
                            obs_tracker_source_plant = ObsTrackerSource.objects.get(target_obs=obs_tracker,
                                                                                    source_obs__obs_entity_type='plant',
                                                                                    relationship='sample_from_plant')
                            obs_tracker_source_plant.source_obs = ObsTracker.objects.get(obs_entity_type='plant',
                                                                                         obs_plant__plant_id=
                                                                                         obs_tracker_samples_form.cleaned_data[
                                                                                             'obs_plant__plant_id'])
                            obs_tracker_source_plant.save()
                        except ObsTrackerSource.DoesNotExist:
                            new_source_plant = ObsTrackerSource.objects.create(
                                source_obs=ObsTracker.objects.get(obs_entity_type='plant', obs_plant__plant_id=
                                obs_tracker_samples_form.cleaned_data['obs_plant__plant_id']), target_obs=obs_tracker,
                                relationship="sample_from_plant")
                    else:
                        obs_tracker.obs_plant = ObsPlant.objects.get(id=1)
                    ...
                    if obs_tracker_samples_form.cleaned_data['source_sample_id'] != '':
                        try:
                            obs_tracker_source_sample = ObsTrackerSource.objects.get(target_obs=obs_tracker,
                                                                                     source_obs__obs_entity_type='sample')
                            obs_tracker_source_sample.source_obs = ObsTracker.objects.get(obs_entity_type='sample',
                                                                                          obs_sample__sample_id=
                                                                                          obs_tracker_samples_form.cleaned_data[
                                                                                              'source_sample_id'])
                            obs_tracker_source_sample.save()
                        except ObsTrackerSource.DoesNotExist:
                            new_source_sample = ObsTrackerSource.objects.create(
                                source_obs=ObsTracker.objects.get(obs_entity_type='sample', obs_sample__sample_id=
                                obs_tracker_samples_form.cleaned_data['source_sample_id']), target_obs=obs_tracker,
                                relationship="sample_from_sample")
                    else:
                        try:
                            obs_tracker_source_sample = ObsTrackerSource.objects.get(target_obs=obs_tracker,
                                                                                     source_obs__obs_entity_type='sample',
        ...
        samples_data = ObsTracker.objects.filter(obs_entity_type='sample', obs_sample_id=obs_sample_id).values(
            'experiment', 'obs_sample__sample_id', 'obs_sample__sample_type', 'obs_sample__sample_name',
            'stock__seed_id', 'obs_plot__plot_id', 'obs_plant__plant_id', 'obs_sample__weight', 'obs_sample__volume',
            'obs_sample__density', 'obs_sample__kernel_num', 'obs_sample__photo', 'obs_sample__comments')
...
def download_tissue_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'Tissue ID', 'Tissue Type', 'Tissue Name', 'Date Ground', 'Comments', 'Plot ID', 'Plant ID',
         'Plate ID', 'Seed ID'])
    for row in tissue_data:
        writer.writerow(
            [row.experiment, row.obs_tissue.tissue_id, row.obs_tissue.tissue_type, row.obs_tissue.tissue_name,
             row.obs_tissue.date_ground, row.obs_tissue.comments, row.obs_plot.plot_id, row.obs_plant.plant_id,
             row.obs_plate.plate_id, row.stock.seed_id])
    return response
...
def suggest_tissue_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        tissue_experiment_list = ObsTracker.objects.filter(obs_entity_type='tissue',
                                                           experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                                 :2000]
    else:
...
def show_all_tissue_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    tissue_experiment_list = ObsTracker.objects.filter(obs_entity_type='tissue').values('experiment__name',
                                                                                        'experiment__field__field_name',
                                                                                        'experiment__field__id',
                                                                                        'experiment__id').distinct()[
                             :2000]
...
def download_tissue_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Tissue ID', 'Tissue Type', 'Tissue Name', 'Date Ground', 'Comments', 'Plot ID', 'Plant ID', 'Plate ID',
         'Seed ID'])
    for row in tissue_data:
        writer.writerow([row.obs_tissue.tissue_id, row.obs_tissue.tissue_type, row.obs_tissue.tissue_name,
                         row.obs_tissue.date_ground, row.obs_tissue.comments, row.obs_plot.plot_id,
                         row.obs_plant.plant_id, row.obs_plate.plate_id, row.stock.seed_id])
    return response
...
def download_separation_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Sample ID', 'Sample Name', 'Separation Type', 'Apparatus', 'SG', 'Light Weight (g)', 'Medium Weight (g)',
         'Heavy Weight (g)', 'Light Percent', 'Medium Percent', 'Heavy Percent', 'Operating Factor', 'Comments'])
    for row in separation_data:
        writer.writerow(
            [row.obs_sample.sample_id, row.obs_sample.sample_name, row.separation_type, row.apparatus, row.SG,
             row.light_weight, row.intermediate_weight, row.heavy_weight, row.light_percent, row.intermediate_percent,
             row.heavy_percent, row.operating_factor, row.comments])
    return response
...
def download_plate_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'Plate ID', 'Plate Name', 'Location', 'Date Plated', 'Contents', 'Rep', 'Plate Type', 'Plate Status',
         'Comments'])
    for row in plate_data:
        writer.writerow(
            [row.experiment, row.obs_plate.plate_id, row.obs_plate.plate_name, row.location, row.obs_plate.date,
             row.obs_plate.contents, row.obs_plate.rep, row.obs_plate.plate_type, row.obs_plate.plate_status,
             row.obs_plate.comments])
    return response
...
def suggest_plate_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        plate_experiment_list = ObsTracker.objects.filter(obs_entity_type='plate',
                                                          experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                                :2000]
    else:
...
def download_plate_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Plate ID', 'Plate Name', 'Date Plate', 'Contents', 'Rep', 'Plate Type', 'Plate Status', 'Plate Comments'])
    for row in plate_data:
        writer.writerow([row.obs_plate.plate_id, row.obs_plate.plate_name, row.obs_plate.date, row.obs_plate.contents,
                         row.obs_plate.rep, row.obs_plate.plate_type, row.obs_plate.plate_status,
                         row.obs_plate.comments])
    return response
...
def download_well_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'Well ID', 'Well', 'Well Inventory', 'Tube Label', 'Comments', 'Plate ID', 'Plot ID', 'Plant ID',
         'Tissue ID', 'Seed ID'])
    for row in well_data:
        writer.writerow([row.experiment, row.obs_well.well_id, row.obs_well.well, row.obs_well.well_inventory,
                         row.obs_well.tube_label, row.obs_well.comments, row.obs_plate.plate_id, row.obs_plot.plot_id,
                         row.obs_plant.plant_id, row.obs_tissue.tissue_id, row.stock.seed_id])
    return response
...
def suggest_well_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        well_experiment_list = ObsTracker.objects.filter(obs_entity_type='well',
                                                         experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                               :2000]
    else:
...
def download_well_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Well ID', 'Well', 'Well Inventory', 'Tube Label', 'Comments', 'Plate ID', 'Plot ID', 'Plant ID', 'Tissue ID',
         'Seed ID'])
    for row in well_data:
        writer.writerow([row.obs_well.well_id, row.obs_well.well, row.obs_well.well_inventory, row.obs_well.tube_label,
                         row.obs_well.comments, row.obs_plate.plate_id, row.obs_plot.plot_id, row.obs_plant.plant_id,
                         row.obs_tissue.tissue_id, row.stock.seed_id])
    return response
...
def download_plant_data(request):
    ...
    for row in plant_data:
        writer.writerow(
            [row.experiment, row.obs_plot, row.stock.seed_id, row.obs_plant.plant_id, row.obs_plant.plant_num,
             row.obs_plant.comments])
    return response
...
def download_plant_experiment(request, experiment_name):
    ...
    for row in plant_data:
        writer.writerow([row.obs_plant.plant_id, row.obs_plant.plant_num, row.obs_plot.plot_id, row.stock.seed_id,
                         row.obs_plant.comments])
    return response
...
def download_culture_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'Culture ID', 'Culture Name', 'Microbe Type', 'Plating Cycle', 'Dilution', 'Image', 'Comments',
         'Medium ID', 'Tissue ID', 'Plant ID', 'Plot ID', 'Seed ID', 'Username'])
    for row in culture_data:
        writer.writerow(
            [row.experiment, row.obs_culture.culture_id, row.obs_culture.culture_name, row.obs_culture.microbe_type,
             row.obs_culture.plating_cycle, row.obs_culture.dilution, row.obs_culture.image_filename,
             row.obs_culture.comments, row.medium.medium_id, row.obs_tissue.tissue_id, row.obs_plot.plot_id,
             row.stock.seed_id, row.user])
    return response
...
def suggest_culture_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        culture_experiment_list = ObsTracker.objects.filter(obs_entity_type='culture',
                                                            experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                                  :2000]
    else:
...
def show_all_culture_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    culture_experiment_list = ObsTracker.objects.filter(obs_entity_type='culture').values('experiment__name',
                                                                                          'experiment__field__field_name',
                                                                                          'experiment__field__id',
                                                                                          'experiment__id').distinct()[
                              :2000]
...
def download_culture_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Culture ID', 'Culture Name', 'Microbe Type', 'Plating Cycle', 'Dilution', 'Image', 'Comments', 'Medium ID',
         'Tissue ID', 'Plant ID', 'Plot ID', 'Seed ID', 'Username'])
    for row in culture_data:
        writer.writerow([row.obs_culture.culture_id, row.obs_culture.culture_name, row.obs_culture.microbe_type,
                         row.obs_culture.plating_cycle, row.obs_culture.dilution, row.obs_culture.image_filename,
                         row.obs_culture.comments, row.medium.medium_id, row.obs_tissue.tissue_id,
                         row.obs_plant.plant_id, row.obs_plot.plot_id, row.stock.seed_id, row.user])
    return response
...
def download_dna_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'DNA ID', 'Extraction Method', 'Date', 'Tube ID', 'Tube Type', 'Comments', 'Well ID', 'Plate ID',
         'Plant ID', 'Tissue ID', 'Plot ID', 'Seed ID', 'Username'])
    for row in dna_data:
        writer.writerow(
            [row.experiment, row.obs_dna.dna_id, row.obs_dna.extraction_method, row.obs_dna.date, row.obs_dna.tube_id,
             row.obs_dna.tube_type, row.obs_dna.comments, row.obs_well.well_id, row.obs_plate.plate_id,
             row.obs_plant.plant_id, row.obs_tissue.tissue_id, row.obs_plot.plot_id, row.stock.seed_id, row.user])
    return response
...
def suggest_dna_experiment(request):
    ...
    if request.method == 'GET':
        ...
        starts_with = request.POST['suggestion']
    if starts_with:
        dna_experiment_list = ObsTracker.objects.filter(obs_entity_type='dna',
                                                        experiment__name__contains=starts_with).values(
            'experiment__name', 'experiment__field__field_name', 'experiment__field__id', 'experiment__id').distinct()[
                              :2000]
    else:
...
def show_all_dna_experiment(request):
    context = RequestContext(request)
    context_dict = {}
    dna_experiment_list = ObsTracker.objects.filter(obs_entity_type='dna').values('experiment__name',
                                                                                  'experiment__field__field_name',
                                                                                  'experiment__field__id',
                                                                                  'experiment__id').distinct()[:2000]
    context_dict = checkbox_session_variable_check(request)
...
def download_dna_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['DNA ID', 'Extraction Method', 'Date', 'Tube ID', 'Tube Type', 'Comments', 'Well ID', 'Plate ID', 'Plant ID',
         'Tissue ID', 'Plot ID', 'Seed ID', 'Username'])
    for row in dna_data:
        writer.writerow([row.obs_dna.dna_id, row.obs_dna.extraction_method, row.obs_dna.date, row.obs_dna.tube_id,
                         row.obs_dna.tube_type, row.obs_dna.comments, row.obs_well.well_id, row.obs_plate.plate_id,
                         row.obs_plant.plant_id, row.obs_tissue.tissue_id, row.obs_plot.plot_id, row.stock.seed_id,
                         row.user])
    return response
            ...
            for e in checkbox_measurement_experiment:
                for p in checkbox_measurement_parameter:
                    m = Measurement.objects.filter(measurement_parameter__parameter=p,
                                                   obs_tracker__experiment__name=e['name'])
...
def download_measurement_data(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Exp ID', 'Obs ID', 'User', 'Time', 'Parameter Type', 'Parameter', 'Value', 'Units', 'Trait ID Buckler',
         'Comments'])
    for row in measurement_data:
        writer.writerow([row.obs_tracker.experiment, row.obs_tracker.obs_id, row.user, row.time_of_measurement,
                         row.measurement_parameter.parameter_type, row.measurement_parameter, row.value,
                         row.measurement_parameter.unit_of_measure, row.measurement_parameter.trait_id_buckler,
                         row.comments])
    return response
    ...
    if starts_with:
        if request.session.get('checkbox_measurement_parameter', None):
            ...
            for parameter in checkbox_parameter_list:
                e = Measurement.objects.filter(obs_tracker__experiment__name__contains=starts_with,
                                               measurement_parameter__parameter=parameter).values(
                    'obs_tracker__experiment_id', 'obs_tracker__experiment__name',
                    'obs_tracker__experiment__field__field_name', 'measurement_parameter__parameter').distinct()
                measurement_experiment_list = list(chain(e, measurement_experiment_list))
            for m in measurement_experiment_list:
                m['input'] = '<input type="checkbox" name="checkbox_measurement_experiment" value="%s">' % (
                m['obs_tracker__experiment_id'])
        else:
            measurement_experiment_list = list(
                Measurement.objects.filter(obs_tracker__experiment__name__contains=starts_with).values(
                    'obs_tracker__experiment_id', 'obs_tracker__experiment__name',
                    'obs_tracker__experiment__field__field_name').distinct())
            for m in measurement_experiment_list:
                m['input'] = '<input type="checkbox" name="checkbox_measurement_experiment" value="%s">' % (
                m['obs_tracker__experiment_id'])
...
def select_measurement_experiment(request):
    ...
    for experiment_id in checkbox_measurement_experiment_list:
        experiment_name = Experiment.objects.filter(id=experiment_id).values('name')
        checkbox_measurement_experiment_name_list = list(
            chain(experiment_name, checkbox_measurement_experiment_name_list))
...
def show_all_measurement_experiment(request):
    ...
    if request.session.get('checkbox_measurement_parameter', None):
        checkbox_parameter_list = request.session.get('checkbox_measurement_parameter')
        for parameter in checkbox_parameter_list:
            e = Measurement.objects.filter(measurement_parameter__parameter=parameter).values(
                'obs_tracker__experiment_id', 'obs_tracker__experiment__name',
                'obs_tracker__experiment__field__field_name', 'measurement_parameter__parameter').distinct()
            measurement_experiment_list = list(chain(e, measurement_experiment_list))
        for m in measurement_experiment_list:
        ...
        measurement_experiment_list = list(
            Measurement.objects.all().values('obs_tracker__experiment_id', 'obs_tracker__experiment__name',
                                             'obs_tracker__experiment__field__field_name').distinct())
        for m in measurement_experiment_list:
            m['input'] = '<input type="checkbox" name="checkbox_measurement_experiment" value="%s">' % (
            m['obs_tracker__experiment_id'])
    ...
    if starts_with:
        if request.session.get('checkbox_measurement_experiment', None):
            ...
            for experiment in checkbox_experiment_list:
                p = Measurement.objects.filter(obs_tracker__experiment__name=experiment['name'],
                                               measurement_parameter__parameter__contains=starts_with).values(
                    'obs_tracker__experiment__name', 'measurement_parameter__parameter_type',
            ...
            measurement_parameter_list = list(
                Measurement.objects.filter(measurement_parameter__parameter__contains=starts_with).values(
                    'measurement_parameter__parameter_type', 'measurement_parameter__parameter').distinct())
            for m in measurement_parameter_list:
                m['input'] = '<input type="checkbox" name="checkbox_measurement_parameter" value="%s">' % (
                m['measurement_parameter__parameter'])
...
def show_all_measurement_parameter(request):
    ...
    if request.session.get('checkbox_measurement_experiment', None):
        checkbox_experiment_list = request.session.get('checkbox_measurement_experiment')
        for experiment in checkbox_experiment_list:
            p = Measurement.objects.filter(obs_tracker__experiment__name=experiment['name']).values(
                'obs_tracker__experiment__name', 'measurement_parameter__parameter_type',
                'measurement_parameter__parameter').distinct()
            measurement_parameter_list = list(chain(p, measurement_parameter_list))
        for m in measurement_parameter_list:
...
def separations_measurement_data_from_experiment(request, experiment_name):
    ...
    context_dict = {}
    measurement_data = find_measurement_from_experiment(experiment_name)
    measurement_data = measurement_data.filter(obs_tracker__obs_entity_type='sample',
                                               measurement_parameter__parameter='AF').exclude(
        obs_tracker__obs_sample__sample_type='Control')
    ...
    for q in measurement_data:
        ...
        data.append({'sample_type_number': unique_types[q.obs_tracker.obs_sample.sample_type],
                     'sample_type': q.obs_tracker.obs_sample.sample_type, 'm_value': q.value,
                     'sample_id': q.obs_tracker.obs_sample.sample_id,
                     'parameter_type': q.measurement_parameter.parameter})
...
def download_measurement_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Obs', 'User', 'Time', 'Parameter Type', 'Parameter', 'Value', 'Units', 'TraitID Buckler', 'Comments'])
    for row in measurement_data:
        writer.writerow(
            [row.obs_tracker.obs_id, row.user, row.time_of_measurement, row.measurement_parameter.parameter_type,
             row.measurement_parameter.parameter, row.value, row.measurement_parameter.unit_of_measure,
             row.measurement_parameter.trait_id_buckler, row.comments])
    return response
...
def download_stockpackets_for_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['Seed ID', 'Seed Name', 'Cross Type', 'Pedigree', 'Population', 'Status', 'Inoculated', 'Seed Comments',
         'Weight (g)', 'Num Seeds', 'Location', 'Packet Comments'])
    for row in stockpackets_used:
        writer.writerow([row.stock.seed_id, row.stock.seed_name, row.stock.cross_type, row.stock.pedigree,
                         row.stock.passport.taxonomy.population, row.stock.stock_status, row.stock.inoculated,
                         row.stock.comments, row.weight, row.num_seeds, row.location.location_name, row.comments])
    return response
...
def download_isolatestocks_experiment(request, experiment_name):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ['IsolateStock ID', 'IsolateStock Name', 'Plant Organ', 'IsolateStock Comments', 'binomial', 'Alias', 'Race',
         'Subtaxa', 'Disease Name', 'Location Name', 'Box Name'])
    for i in isolatestock_data:
        writer.writerow(
            [i.isolatestock_id, i.isolatestock_name, i.plant_organ, i.comments, i.passport.taxonomy.binomial,
             i.passport.taxonomy.alias, i.passport.taxonomy.race, i.passport.taxonomy.subtaxa, i.disease_info,
             i.location.location_name, i.location.box_name])
    return response
                    ...
                    try:
                        ...
                        with transaction.atomic():
                            try:
                                ...
                                new_taxonomy, created = Taxonomy.objects.get_or_create(binomial=binomial,
                                                                                       population=population,
                                                                                       common_name='Maize',
                                                                                       alias='NULL', race='NULL',
                                                                                       subtaxa='NULL')
                                new_collecting, created = Collecting.objects.get_or_create(user=collection_user,
                                                                                           collection_date=collection_date,
                                                                                           collection_method=collection_method,
                                                                                           comments=collection_comments)
                                new_passport, created = Passport.objects.get_or_create(collecting=new_collecting,
                                                                                       taxonomy=new_taxonomy,
                                                                                       people=new_people)
                                new_stock, created = Stock.objects.get_or_create(passport=new_passport, seed_id=seed_id,
                                                                                 seed_name=seed_name,
                                ...
                                if experiment_collected == True:
                                    new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='stock',
                                                                                                ...
                                                                                                obs_tissue_id=1,
                                                                                                obs_well_id=1)
                                    new_obs_tracker_exp, created = ObsTracker.objects.get_or_create(
                                        obs_entity_type='experiment', stock_id=1, experiment=experiment, user=user,
                                        field_id=1, isolate_id=1, isolatestock_id=1, location_id=1, maize_sample_id=1,
                                        obs_culture_id=1, obs_dna_id=1, obs_env_id=1, obs_extract_id=1,
                                        obs_microbe_id=1, obs_plant_id=1, obs_plate_id=1, obs_plot_id=1,
                                        obs_sample_id=1, obs_tissue_id=1, obs_well_id=1)
                                    new_obs_tracker_source, created = ObsTrackerSource.objects.get_or_create(
                                        source_obs=new_obs_tracker_exp, target_obs=new_obs_tracker,
                                        relationship='stock_from_experiment')
                                    if plot_id != '':
                                        source_plot, created = ObsTracker.objects.get_or_create(obs_entity_type='plot',
                                                                                                obs_plot=obs_plot)
                                        new_obs_tracker_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                            source_obs=source_plot, target_obs=new_obs_tracker,
                                            relationship='stock_from_plot')
                                    if plant_id != '':
                                        source_plant, created = ObsTracker.objects.get_or_create(
                                            obs_entity_type='plant', obs_plant=obs_plant)
                                        new_obs_tracker_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                            source_obs=source_plant, target_obs=new_obs_tracker,
                                            relationship='stock_from_plant')
                            except Exception as e:
                                print("Error: %s %s" % (e.message, e.args))
                                failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_field = Field.objects.get_or_create(locality=locality, field_name=field_name,
                                                                    field_num=field_num, comments=comments,
                                                                    planting_year=planting_year, dimensions=dimensions)
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_locality, created = Locality.objects.get_or_create(city=city, county=county,
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_parameter = MeasurementParameter.objects.get_or_create(parameter=parameter,
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_location = Location.objects.get_or_create(locality=locality,
                                                                          building_name=building_name,
                                                                          location_name=location_name, room=room,
                                                                          shelf=shelf, column=column, box_name=box_name,
                                                                          comments=comments)
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_disease = DiseaseInfo.objects.get_or_create(common_name=common_name, abbrev=abbrev,
                                                                            comments=comments)
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_citation = Citation.objects.get_or_create(citation_type=citation_type, title=title,
                                                                          url=url, pubmed_id=pubmed_id,
                                                                          comments=comments)
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        if plot_id != '':
                            try:
                                obs_tracker = ObsTracker.objects.get(obs_entity_type='plot',
                                                                     obs_plot=ObsPlot.objects.get(plot_id=plot_id))
                            ...
                            obs_plot_id = 1
                        try:
                            new_obsplant, created = ObsPlant.objects.get_or_create(plant_id=plant_id,
                                                                                   plant_num=plant_num,
                                                                                   comments=plant_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='plant',
                                                                                        stock=Stock.objects.get(
                                                                                            seed_id=seed_id),
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=field_id,
                                                                                        ...
                                                                                        obs_microbe_id=1,
                                                                                        obs_plant=new_obsplant,
                                                                                        obs_plate_id=1,
                                                                                        obs_plot_id=obs_plot_id,
                                                                                        obs_sample_id=1,
                                                                                        obs_tissue_id=1, obs_well_id=1)
                            if plot_id != '':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=plot_id),
                                    target_obs=new_obs_tracker, relationship="plant_from_plot")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
                        ...
                        try:
                            ...
                            if seed_id != '' and seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    ...
                                    target_obs=new_obs_tracker, relationship="row_from_stock")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obsenv, created = ObsEnv.objects.get_or_create(environment_id=environment_id,
                                                                               longitude=longitude, latitude=latitude,
                                                                               comments=environment_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='environment',
                                                                                        stock_id=1,
                                                                                        experiment=experiment,
                                                                                        user=user, field=field,
                                                                                        isolate_id=1, isolatestock_id=1,
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obssample, created = ObsSample.objects.get_or_create(sample_id=sample_id,
                                                                                     sample_type=sample_type,
                                                                                     sample_name=sample_name,
                                                                                     weight=weight, volume=volume,
                                                                                     density=density,
                                                                                     kernel_num=kernel_num, photo=photo,
                                                                                     comments=sample_comments)
                            new_obs_tracker, new_obs_tracker_created = ObsTracker.objects.get_or_create(
                                obs_entity_type='sample', stock=Stock.objects.get(seed_id=source_seed_id),
                                experiment=experiment, user=user, field_id=1, isolate_id=1, isolatestock_id=1,
                                location_id=1, maize_sample_id=1, obs_culture_id=1, obs_dna_id=1, obs_env_id=1,
                                obs_extract_id=1, obs_microbe_id=1,
                                obs_plant=ObsPlant.objects.get(plant_id=source_plant_id), obs_plate_id=1,
                                obs_plot=ObsPlot.objects.get(plot_id=source_plot_id), obs_sample=new_obssample,
                                obs_tissue_id=1, obs_well_id=1)
                            if source_sample_id != '' and source_sample_id != 'No Sample':
                                new_source_sample, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='sample',
                                                                      obs_sample__sample_id=source_sample_id),
                                    target_obs=new_obs_tracker, relationship='sample_from_sample')
                            if source_plot_id != '' and source_plot_id != 'No Plot':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=source_plot_id),
                                    target_obs=new_obs_tracker, relationship="sample_from_plot")
                            if source_plant_id != '' and source_plant_id != 'No Plant':
                                new_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plant',
                                                                      obs_plant__plant_id=source_plant_id),
                                    target_obs=new_obs_tracker, relationship="sample_from_plant")
                            if source_seed_id != '' and source_seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='stock',
                                                                      stock__seed_id=source_seed_id),
                                    target_obs=new_obs_tracker, relationship="sample_from_stock")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obstissue, created = ObsTissue.objects.get_or_create(tissue_id=tissue_id,
                                                                                     tissue_name=tissue_name,
                                                                                     tissue_type=tissue_type,
                                                                                     date_ground=date_ground,
                                                                                     comments=tissue_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='tissue',
                                                                                        stock=Stock.objects.get(
                                                                                            seed_id=seed_id),
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=1,
                                                                                        isolate_id=1, isolatestock_id=1,
                                                                                        location_id=1,
                                                                                        maize_sample_id=1,
                                                                                        obs_culture=ObsCulture.objects.get(
                                                                                            culture_id=culture_id),
                                                                                        obs_dna_id=1, obs_env_id=1,
                                                                                        obs_extract_id=1,
                                                                                        obs_microbe_id=1,
                                                                                        obs_plant=ObsPlant.objects.get(
                                                                                            plant_id=plant_id),
                                                                                        obs_plate_id=1,
                                                                                        obs_plot=ObsPlot.objects.get(
                                                                                            plot_id=plot_id),
                                                                                        obs_sample_id=1,
                                                                                        obs_tissue=new_obstissue,
                                                                                        obs_well_id=1)
                            if plot_id != '' and plot_id != 'No Plot':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=plot_id),
                                    target_obs=new_obs_tracker, relationship='tissue_from_plot')
                            if culture_id != '' and culture_id != 'No Culture':
                                new_source_culture, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='culture',
                                                                      obs_culture__culture_id=culture_id),
                                    target_obs=new_obs_tracker, relationship="tissue_from_culture")
                            if plant_id != '' and plant_id != 'No Plant':
                                new_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plant',
                                                                      obs_plant__plant_id=plant_id),
                                    target_obs=new_obs_tracker, relationship="tissue_from_plant")
                            if seed_id != '' and seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='stock', stock__seed_id=seed_id),
                                    target_obs=new_obs_tracker, relationship="tissue_from_stock")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obsculture, created = ObsCulture.objects.get_or_create(medium=medium,
                                                                                       culture_id=culture_id,
                                                                                       culture_name=culture_name,
                                                                                       microbe_type=microbe_type,
                                                                                       plating_cycle=plating_cycle,
                                                                                       dilution=dilution,
                                                                                       image_filename=image,
                                                                                       num_colonies=num_colonies,
                                                                                       num_microbes=num_microbes,
                                                                                       comments=tissue_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='culture',
                                                                                        stock=Stock.objects.get(
                                                                                            seed_id=seed_id),
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=1,
                                                                                        ...
                                                                                        location_id=1,
                                                                                        maize_sample_id=1,
                                                                                        obs_culture=new_obsculture,
                                                                                        obs_dna_id=1, obs_env_id=1,
                                                                                        obs_extract_id=1,
                                                                                        obs_microbe=ObsMicrobe.objects.get(
                                                                                        ...
                                                                                        obs_plot=ObsPlot.objects.get(
                                                                                            plot_id=plot_id),
                                                                                        obs_sample=new_obssample,
                                                                                        obs_tissue=ObsTissue.objects.get(
                                                                                            tissue_id=tissue_id),
                                                                                        obs_well_id=1)
                            if plot_id != '' and plot_id != 'No Plot':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=plot_id),
                                    target_obs=new_obs_tracker, relationship='culture_from_plot')
                            if tissue_id != '' and tissue_id != 'No Tissue':
                                new_source_tissue, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='tissue',
                                                                      obs_tissue__tissue_id=tissue_id),
                                    target_obs=new_obs_tracker, relationship="culture_from_tissue")
                            if plant_id != '' and plant_id != 'No Plant':
                                new_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plant',
                                                                      obs_plant__plant_id=plant_id),
                                    target_obs=new_obs_tracker, relationship="culture_from_plant")
                            if seed_id != '' and seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='stock', stock__seed_id=seed_id),
                                    target_obs=new_obs_tracker, relationship="culture_from_stock")
                            if microbe_id != '' and microbe_id != 'No Microbe':
                                new_source_microbe, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='microbe',
                                                                      obs_microbe__microbe_id=microbe_id),
                                    target_obs=new_obs_tracker, relationship="culture_from_microbe")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obsmicrobe, created = ObsMicrobe.objects.get_or_create(microbe_id=microbe_id,
                                                                                       microbe_type=microbe_type,
                                                                                       comments=microbe_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='microbe',
                                                                                        stock=Stock.objects.get(
                                                                                            seed_id=seed_id),
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=1,
                            ...
                            if plot_id != '' and plot_id != 'No Plot':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=plot_id),
                                    target_obs=new_obs_tracker, relationship='microbe_from_plot')
                            if tissue_id != '' and tissue_id != 'No Tissue':
                                new_source_tissue, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='tissue',
                                                                      obs_tissue__tissue_id=tissue_id),
                                    target_obs=new_obs_tracker, relationship="microbe_from_tissue")
                            if plant_id != '' and plant_id != 'No Plant':
                                new_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plant',
                                                                      obs_plant__plant_id=plant_id),
                                    target_obs=new_obs_tracker, relationship="microbe_from_plant")
                            if seed_id != '' and seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='stock', stock__seed_id=seed_id),
                                    target_obs=new_obs_tracker, relationship="microbe_from_stock")
                            if culture_id != '' and culture_id != 'No Culture':
                                new_source_culture, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='culture',
                                                                      obs_culture__culture_id=culture_id),
                                    target_obs=new_obs_tracker, relationship="microbe_from_culture")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obsdna, created = ObsDNA.objects.get_or_create(dna_id=dna_id,
                                                                               extraction_method=extraction, date=date,
                                                                               tube_id=tube_id, tube_type=tube_type,
                                                                               comments=dna_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='dna',
                                                                                        stock=Stock.objects.get(
                                                                                            seed_id=seed_id),
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=1,
                                                                                        ...
                                                                                        obs_plant=ObsPlant.objects.get(
                                                                                            plant_id=plant_id),
                                                                                        obs_plate=ObsPlate.objects.get(
                                                                                            plate_id=plate_id),
                                                                                        obs_plot=ObsPlot.objects.get(
                                                                                            plot_id=plot_id),
                                                                                        obs_sample=ObsSample.objects.get(
                                                                                            sample_id=sample_id),
                                                                                        obs_tissue=ObsTissue.objects.get(
                                                                                            tissue_id=tissue_id),
                                                                                        obs_well=ObsWell.objects.get(
                                                                                            well_id=well_id))
                            if plot_id != '' and plot_id != 'No Plot':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=plot_id),
                                    target_obs=new_obs_tracker, relationship='dna_from_plot')
                            if tissue_id != '' and tissue_id != 'No Tissue':
                                new_source_tissue, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='tissue',
                                                                      obs_tissue__tissue_id=tissue_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_tissue")
                            if plant_id != '' and plant_id != 'No Plant':
                                new_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plant',
                                                                      obs_plant__plant_id=plant_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_plant")
                            if seed_id != '' and seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='stock', stock__seed_id=seed_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_stock")
                            if culture_id != '' and culture_id != 'No Culture':
                                new_source_culture, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='culture',
                                                                      obs_culture__culture_id=culture_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_culture")
                            if microbe_id != '' and microbe_id != 'No Microbe':
                                new_source_microbe, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='microbe',
                                                                      obs_microbe__microbe_id=microbe_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_microbe")
                            if plate_id != '' and plate_id != 'No Plate':
                                new_source_plate, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plate',
                                                                      obs_plate__plate_id=plate_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_plate")
                            if well_id != '' and well_id != 'No Well':
                                new_source_well, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='well',
                                                                      obs_well__well_id=well_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_well")
                            if sample_id != '' and sample_id != 'No Sample':
                                new_source_sample, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='sample',
                                                                      obs_sample__sample_id=sample_id),
                                    target_obs=new_obs_tracker, relationship="dna_from_sample")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obsplate, created = ObsPlate.objects.get_or_create(plate_id=plate_id,
                                                                                   plate_name=plate_name, date=date,
                                                                                   contents=contents, rep=rep,
                                                                                   plate_type=plate_type,
                                                                                   plate_status=plate_status,
                                                                                   comments=plate_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='plate',
                                                                                        stock_id=1,
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=1,
                                                                                        isolate_id=1, isolatestock_id=1,
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_obswell, created = ObsWell.objects.get_or_create(well_id=well_id, well=well, date=date,
                                                                                 well_inventory=inventory,
                                                                                 tube_label=tube_label,
                                                                                 comments=well_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='well',
                                                                                        stock=Stock.objects.get(
                                                                                            seed_id=seed_id),
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=1,
                                                                                        ...
                                                                                        obs_tissue=ObsTissue.objects.get(
                                                                                            tissue_id=tissue_id),
                                                                                        obs_well=new_obswell)
                            if plot_id != '' and plot_id != 'No Plot':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=plot_id),
                                    target_obs=new_obs_tracker, relationship='well_from_plot')
                            if tissue_id != '' and tissue_id != 'No Tissue':
                                new_source_tissue, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='tissue',
                                                                      obs_tissue__tissue_id=tissue_id),
                                    target_obs=new_obs_tracker, relationship="well_from_tissue")
                            if plant_id != '' and plant_id != 'No Plant':
                                new_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plant',
                                                                      obs_plant__plant_id=plant_id),
                                    target_obs=new_obs_tracker, relationship="well_from_plant")
                            if seed_id != '' and seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='stock', stock__seed_id=seed_id),
                                    target_obs=new_obs_tracker, relationship="well_from_stock")
                            if culture_id != '' and culture_id != 'No Culture':
                                new_source_culture, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='culture',
                                                                      obs_culture__culture_id=culture_id),
                                    target_obs=new_obs_tracker, relationship="well_from_culture")
                            if microbe_id != '' and microbe_id != 'No Microbe':
                                new_source_microbe, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='microbe',
                                                                      obs_microbe__microbe_id=microbe_id),
                                    target_obs=new_obs_tracker, relationship="well_from_microbe")
                            if plate_id != '' and plate_id != 'No Plate':
                                new_source_plate, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plate',
                                                                      obs_plate__plate_id=plate_id),
                                    target_obs=new_obs_tracker, relationship="well_from_plate")
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_separation, created = Separation.objects.get_or_create(
                                obs_sample=ObsSample.objects.get(sample_id=sample_id), separation_type=separation_type,
                                apparatus=apparatus, SG=sg, light_weight=light_weight,
                                intermediate_weight=medium_weight, heavy_weight=heavy_weight,
                                light_percent=light_percent, intermediate_percent=medium_percent,
                                heavy_percent=heavy_percent, operating_factor=operating_factor,
                                comments=separation_comments)
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_maize_sample, created = MaizeSample.objects.get_or_create(maize_id=maize_id,
                                                                                          county=county,
                                                                                          ...
                                                                                          photo=photo)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='maize',
                                                                                        stock_id=1,
                                                                                        experiment=experiment,
                                                                                        user=user, field_id=1,
                                                                                        isolate_id=1, isolatestock_id=1,
                                                                                        location_id=1,
                                                                                        maize_sample=new_maize_sample,
                                                                                        obs_culture_id=1, obs_dna_id=1,
                                                                                        obs_env_id=1, obs_extract_id=1,
                                                                                        obs_microbe_id=1,
                                                                                        obs_plant_id=1, obs_plate_id=1,
                    ...
                    try:
                        ...
                        try:
                            new_people, created = People.objects.get_or_create(first_name=source_fname,
                                                                               last_name=source_lname,
                                                                               organization=source_organization,
                                                                               phone=source_phone, email=source_email,
                                                                               comments=source_comments)
                            new_collecting, created = Collecting.objects.get_or_create(user=collection_user,
                                                                                       collection_date=collection_date,
                                                                                       collection_method=collection_method,
                                                                                       comments=collection_comments)
                            new_taxonomy, created = Taxonomy.objects.get_or_create(binomial=binomial,
                                                                                   common_name='IsolateStock',
                                                                                   alias=alias, race=race,
                                                                                   subtaxa=subtaxa)
                            new_passport, created = Passport.objects.get_or_create(taxonomy=new_taxonomy,
                                                                                   people=new_people,
                                                                                   collecting=new_collecting)
                            new_isolatestock, created = IsolateStock.objects.get_or_create(passport=new_passport,
                                                                                           locality=locality,
                                                                                           disease_info_id=1,
                                                                                           isolatestock_id=isolatestock_id,
                                                                                           isolatestock_name=isolatestock_name,
                                                                                           plant_organ=plant_organ,
                                                                                           comments=isolatestock_comments)
                            new_obs_tracker, created = ObsTracker.objects.get_or_create(obs_entity_type='isolatestock',
                                                                                        stock=Stock.objects.get(
                                                                                            seed_id=seed_id),
                                                                                        experiment_id=1, user=user,
                                                                                        field=field, isolate_id=1,
                                                                                        isolatestock=new_isolatestock,
                                                                                        location_id=1,
                                                                                        maize_sample_id=1, obs_env_id=1,
                                                                                        obs_extract_id=1,
                                                                                        obs_plant=ObsPlant.objects.get(
                                                                                            plant_id=plant_id),
                                                                                        obs_plot=ObsPlot.objects.get(
                            ...
                            if plot_id != '' and plot_id != 'No Plot':
                                new_source_plot, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plot',
                                                                      obs_plot__plot_id=plot_id),
                                    target_obs=new_obs_tracker, relationship='isolatestock_from_plot')
                            if tissue_id != '' and tissue_id != 'No Tissue':
                                new_source_tissue, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='tissue',
                                                                      obs_tissue__tissue_id=tissue_id),
                                    target_obs=new_obs_tracker, relationship="isolatestock_from_tissue")
                            if plant_id != '' and plant_id != 'No Plant':
                                new_source_plant, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='plant',
                                                                      obs_plant__plant_id=plant_id),
                                    target_obs=new_obs_tracker, relationship="isolatestock_from_plant")
                            if seed_id != '' and seed_id != 'No Stock':
                                new_source_stock, created = ObsTrackerSource.objects.get_or_create(
                                    source_obs=ObsTracker.objects.get(obs_entity_type='stock', stock__seed_id=seed_id),
                                    target_obs=new_obs_tracker, relationship="isolatestock_from_stock")
                        except Exception as e:
                            print("Error: %s %s" % (e.message, e.args))
                            failed = True
                    except KeyError:
            ...
            if log_data_online_form_set.is_valid():
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            new_isolate, created = Isolate.objects.get_or_create(isolate_id=isolate_id,
                                                                                 isolatestock=isolatestock,
                                                                                 location=location, locality=locality,
                                                                                 stock_date=stock_date,
                                                                                 extract_color=extract_color,
                                                                                 organism=organism,
                                                                                 comments=isolate_comments, user=user)
                            new_isolate.save()
                ...
                for form in log_data_online_form_set:
                    try:
                        ...
                        try:
                            ...
                            obs_tracker = ObsTracker.objects.get(obs_entity_type='plot', obs_plot=obs_unit)
                        ...
                        if obs_tracker_found == True:
                            try:
                                new_measurement, created = Measurement.objects.get_or_create(obs_tracker=obs_tracker,
                                                                                             measurement_parameter=measurement_parameter,
                                                                                             user=user,
                                                                                             time_of_measurement=time_of_measurement,
                                                                                             value=value,
                                                                                             comments=measurement_comments,
                                                                                             experiment=experiment)
                            except Exception as e:
                                print("Error: %s %s" % (e.message, e.args))
                                failed = True
...
def queue_upload_file(request, data_type):
    ...
    if request.method == 'POST':
        ...
        if upload_form.is_valid():
            ...
            new_upload = UploadQueue.objects.get_or_create(experiment=new_upload_exp, user=new_upload_user,
                                                           file_name=new_upload_filename, upload_type=data_type,
                                                           comments=new_upload_comments)
...
def sidebar_search_page(request):
    ...
    try:
        isolatestock_results = IsolateStock.objects.filter(isolatestock_id__icontains=starts_with).exclude(
    ...
    try:
        env_results = ObsEnv.objects.filter(environment_id__icontains=starts_with).exclude(
...
def sidebar_search(request):
    ...
    if starts_with:
        try:
            experiment_results = Experiment.objects.filter(name__contains=starts_with).exclude(name='No Experiment')[
                                 :10]
        except Experiment.DoesNotExist:
            experiment_results = None
        for result in experiment_results:
        ...
        for result in row_results:
            result.url = "/lab/plot/%d/" % (result.id)
        try:
            isolatestock_results = IsolateStock.objects.filter(isolatestock_id__contains=starts_with).exclude(
                isolatestock_id='No IsolateStock')[:10]
        except IsolateStock.DoesNotExist:
            isolatestock_results = None
        for result in isolatestock_results:
            result.url = "/lab/isolatestock/%d/" % (result.id)
        ...
        for result in plate_results:
            result.url = "/lab/plate/%d/" % (result.id)
        try:
            culture_results = ObsCulture.objects.filter(culture_id__contains=starts_with).exclude(
                culture_id='No Culture')[:10]
        except ObsCulture.DoesNotExist:
            culture_results = None
        for result in culture_results:
            result.url = "/lab/culture/%d/" % (result.id)
        try:
            microbe_results = ObsMicrobe.objects.filter(microbe_id__contains=starts_with).exclude(
                microbe_id='No Microbe')[:10]
        except ObsMicrobe.DoesNotExist:
            microbe_results = None
        for result in microbe_results:
            result.url = "/lab/microbe/%d/" % (result.id)
        try:
            extract_results = ObsExtract.objects.filter(extract_id__contains=starts_with).exclude(
                extract_id='No Extract')[:10]
        except ObsExtract.DoesNotExist:
            extract_results = None
        for result in extract_results:
            result.url = "/lab/extract/%d/" % (result.id)
        ...
        for result in dna_results:
            result.url = "/lab/dna/%d/" % (result.id)
        try:
            sample_results = ObsSample.objects.filter(sample_id__contains=starts_with).exclude(sample_id='No Sample')[
                             :10]
        except ObsSample.DoesNotExist:
            sample_results = None
        for result in sample_results:
            result.url = "/lab/sample/%d/" % (result.id)
        try:
            tissue_results = ObsTissue.objects.filter(tissue_id__contains=starts_with).exclude(tissue_id='No Tissue')[
                             :10]
        except ObsTissue.DoesNotExist:
            tissue_results = None
        for result in tissue_results:
            result.url = "/lab/tissue/%d/" % (result.id)
        try:
            env_results = ObsEnv.objects.filter(environment_id__contains=starts_with).exclude(
                environment_id='No Environment')[:10]
        except ObsEnv.DoesNotExist:
            env_results = None
        for result in env_results:
            result.url = "/lab/env/%d/" % (result.id)
        ...
        for result in well_results:
            result.url = "/lab/well/%d/" % (result.id)
        try:
            maize_results = MaizeSample.objects.filter(maize_id__contains=starts_with).exclude(maize_id='No Maize ID')[
                            :10]
        except MaizeSample.DoesNotExist:
            maize_results = None
        for result in maize_results:
        ...
        results_list = list(
            chain(experiment_results, user_results, row_results, stock_results, isolatestock_results, isolate_results,
                  plant_results, plate_results, culture_results, microbe_results, extract_results, dna_results,
                  sample_results, tissue_results, env_results, well_results, maize_results, field_results,
                  locality_results, people_results))[:10]
...
def query_builder_options(request):
    ...
    if 'Measurement' in selected_options:
        measurement_fields_list = [('measurement_time',
                                    "<select name='qb_measurement_time_choice'><option value='' >None</option><option value='time_of_measurement'>Ascending</option><option value='-time_of_measurement'>Descending</option></select>",
                                    '<input class="search-query" type="text" name="qb_measurement_time" placeholder="Type a Date"/>',
                                    'checkbox_qb_measurement'), ('measurement_value',
                                                                 "<select name='qb_measurement_value_choice'><option value='' >None</option><option value='value'>Ascending</option><option value='-value'>Descending</option></select>",
                                                                 '<input class="search-query" type="text" name="qb_measurement_value" placeholder="Type a Value"/>',
                                                                 'checkbox_qb_measurement'), ('measurement_comments',
                                                                                              "<select name='qb_measurement_comments_choice'><option value='' >None</option><option value='comments'>A to Z</option><option value='-comments'>Z to A</option></select>",
                                                                                              '<input class="search-query" type="text" name="qb_measurement_comments" placeholder="Type a Comment"/>',
                                                                                              'checkbox_qb_measurement'),
                                   ('measurement_parameter',
                                    "<select name='qb_measurement_parameter_choice'><option value='' >None</option><option value='measurement_parameter__parameter'>A to Z</option><option value='-measurement_parameter__parameter'>Z to A</option></select>",
                                    '<input class="search-query" type="text" name="qb_measurement_parameter" placeholder="Type a Parameter"/>',
                                    'checkbox_qb_measurement'), ('measurement_parameter_type',
                                                                 "<select name='qb_measurement_type_choice'><option value='' >None</option><option value='measurement_parameter__parameter_type'>A to Z</option><option value='-measurement_parameter__parameter_type'>Z to A</option></select>",
                                                                 '<input class="search-query" type="text" name="qb_measurement_type" placeholder="Type a Parameter Type"/>',
                                                                 'checkbox_qb_measurement'), ('measurement_protocol',
                                                                                              "<select name='qb_measurement_protocol_choice'><option value='' >None</option><option value='measurement_parameter__protocol'>A to Z</option><option value='-measurement_parameter__protocol'>Z to A</option></select>",
                                                                                              '<input class="search-query" type="text" name="qb_measurement_protocol" placeholder="Type a Protocol"/>',
                                                                                              'checkbox_qb_measurement'),
                                   ('measurement_unit_of_measure',
                                    "<select name='qb_measurement_unit_choice'><option value='' >None</option><option value='measurement_parameter__unit_of_measure'>A to Z</option><option value='-measurement_parameter__unit_of_measure'>Z to A</option></select>",
                                    '<input class="search-query" type="text" name="qb_measurement_unit" placeholder="Type a Unit"/>',
                                    'checkbox_qb_measurement'), ('measurement_trait_id_buckler',
                                                                 "<select name='qb_measurement_buckler_choice'><option value='' >None</option><option value='measurement_parameter__trait_id_buckler'>A to Z</option><option value='-measurement_parameter__trait_id_buckler'>Z to A</option></select>",
                                                                 '<input class="search-query" type="text" name="qb_measurement_buckler" placeholder="Type a Buckler TraitID"/>',
                                                                 'checkbox_qb_measurement')]
        query_builder_fields_list = list(chain(measurement_fields_list, query_builder_fields_list))
    if 'Stock' in selected_options:
        stock_fields_list = [('stock_seed_id',
                              "<select name='qb_stock_seed_id_choice'><option value='' >None</option><option value='stock__seed_id'>Ascending</option><option value='-stock__seed_id'>Descending</option></select>",
                              '<input class="search-query" type="text" name="qb_stock_seed_id" placeholder="Type a Seed ID"/>',
                              'checkbox_qb_stock'), ('stock_seed_name',
                                                     "<select name='qb_stock_seed_name_choice'><option value='' >None</option><option value='stock__seed_name'>A to Z</option><option value='-stock__seed_name'>Z to A</option></select>",
                                                     '<input class="search-query" type="text" name="qb_stock_seed_name" placeholder="Type a Seed Name"/>',
                                                     'checkbox_qb_stock'), ('stock_cross_type',
                                                                            "<select name='qb_stock_cross_choice'><option value='' >None</option><option value='stock__cross_type'>A to Z</option><option value='-stock__cross_type'>Z to A</option></select>",
                                                                            '<input class="search-query" type="text" name="qb_stock_cross" placeholder="Type a Cross Type"/>',
                                                                            'checkbox_qb_stock'), ('stock_pedigree',
                                                                                                   "<select name='qb_stock_pedigree_choice'><option value='' >None</option><option value='stock__pedigree'>A to Z</option><option value='-stock__pedigree'>Z to A</option></select>",
                                                                                                   '<input class="search-query" type="text" name="qb_stock_pedigree" placeholder="Type a Pedigree"/>',
                                                                                                   'checkbox_qb_stock'),
                             ('stock_status',
                              "<select name='qb_stock_status_choice'><option value='' >None</option><option value='stock__stock_status'>A to Z</option><option value='-stock__stock_status'>Z to A</option></select>",
                              '<input class="search-query" type="text" name="qb_stock_status" placeholder="Type a Stock Status"/>',
                              'checkbox_qb_stock'), ('stock_date',
                                                     "<select name='qb_stock_date_choice'><option value='' >None</option><option value='stock__stock_date'>Ascending</option><option value='-stock__stock_date'>Descending</option></select>",
                                                     '<input class="search-query" type="text" name="qb_stock_date" placeholder="Type a Stock Date"/>',
                                                     'checkbox_qb_stock'), (
                             'stock_inoculated', '', '<input type="checkbox" name="qb_stock_inoculated" value="1"/>',
                             'checkbox_qb_stock'), ('stock_comments',
                                                    "<select name='qb_stock_comments_choice'><option value='' >None</option><option value='stock__comments'>A to Z</option><option value='-stock__comments'>Z to A</option></select>",
                                                    '<input class="search-query" type="text" name="qb_stock_comments" placeholder="Type Stock Comments"/>',
                                                    'checkbox_qb_stock'), ('stock_binomial',
                                                                           "<select name='qb_stock_binomial_choice'><option value='' >None</option><option value='stock__passport__taxonomy__binomial'>A to Z</option><option value='-stock__passport__taxonomy__binomial'>Z to A</option></select>",
                                                                           '<input class="search-query" type="text" name="qb_stock_binomial" placeholder="Type a binomial"/>',
                                                                           'checkbox_qb_stock'), ('checkbox_qb_stock'),
                             ('stock_population',
                              "<select name='qb_stock_population_choice'><option value='' >None</option><option value='stock__passport__taxonomy__population'>A to Z</option><option value='-stock__passport__taxonomy__population'>Z to A</option></select>",
                              '<input class="search-query" type="text" name="qb_stock_population" placeholder="Type a Population"/>',
                              'checkbox_qb_stock'), ('stock_collection_date',
                                                     "<select name='qb_stock_collecting_date_choice'><option value='' >None</option><option value='stock__passport__collecting__collection_date'>Ascending</option><option value='-stock__passport__collecting__collection_date'>Descending</option></select>",
                                                     '<input class="search-query" type="text" name="qb_stock_collecting_date" placeholder="Type a Collection Date"/>',
                                                     'checkbox_qb_stock'), ('stock_collection_method',
                                                                            "<select name='qb_stock_collecting_method_choice'><option value='' >None</option><option value='stock__passport__collecting__collection_method'>A to Z</option><option value='-stock__passport__collecting__collection_method'>Z to A</option></select>",
                                                                            '<input class="search-query" type="text" name="qb_stock_collecting_method" placeholder="Type a Collection Method"/>',
                                                                            'checkbox_qb_stock'), (
                             'stock_collection_comments',
                             "<select name='qb_stock_collecting_comments_choice'><option value='' >None</option><option value='stock__passport__collecting__comments'>A to Z</option><option value='-stock__passport__collecting__comments'>Z to A</option></select>",
                             '<input class="search-query" type="text" name="qb_stock_collecting_comments" placeholder="Type Collecting Comments"/>',
                             'checkbox_qb_stock'), ('stock_source_organization',
                                                    "<select name='qb_stock_people_org_choice'><option value='' >None</option><option value='stock__passport__people__organization'>A to Z</option><option value='-stock__passport__people__organization'>Z to A</option></select>",
                                                    '<input class="search-query" type="text" name="qb_stock_people_org" placeholder="Type a Source"/>',
                                                    'checkbox_qb_stock')]
        query_builder_fields_list = list(chain(stock_fields_list, query_builder_fields_list))
    if 'Experiment' in selected_options:
        experiment_fields_list = [('experiment_name',
                                   "<select name='qb_experiment_name_choice'><option value='' >None</option><option value='experiment__name'>Ascending</option><option value='-experiment__name'>Descending</option></select>",
                                   '<input class="search-query" type="text" name="qb_experiment_name" placeholder="Type Experiment Name"/>',
                                   'checkbox_qb_experiment'), ('experiment_field_name',
                                                               "<select name='qb_experiment_field_name_choice'><option value='' >None</option><option value='experiment__field__field_name'>A to Z</option><option value='-experiment__field__field_name'>Z to A</option></select>",
                                                               '<input class="search-query" type="text" name="qb_experiment_field_name" placeholder="Type a Field Name"/>',
                                                               'checkbox_qb_experiment'), (
                                  'experiment_field_locality_city',
                                  "<select name='qb_experiment_field_locality_city_choice'><option value='' >None</option><option value='experiment__field__locality__city'>A to Z</option><option value='-experiment__field__locality__city'>Z to A</option></select>",
                                  '<input class="search-query" type="text" name="qb_experiment_field_locality_city" placeholder="Type a Locality City"/>',
                                  'checkbox_qb_experiment'), ('experiment_start_date',
                                                              "<select name='qb_experiment_date_choice'><option value='' >None</option><option value='experiment__start_date'>Ascending</option><option value='-experiment__start_date'>Descending</option></select>",
                                                              '<input class="search-query" type="text" name="qb_experiment_date" placeholder="Type an Experiment Date"/>',
                                                              'checkbox_qb_experiment'), ('experiment_purpose',
                                                                                          "<select name='qb_experiment_purpose_choice'><option value='' >None</option><option value='experiment__purpose'>A to Z</option><option value='-experiment__purpose'>Z to A</option></select>",
                                                                                          '<input class="search-query" type="text" name="qb_experiment_purpose" placeholder="Type Experiment Purpose"/>',
                                                                                          'checkbox_qb_experiment'), (
                                  'experiment_comments',
                                  "<select name='qb_experiment_comments_choice'><option value='' >None</option><option value='experiment__comments'>A to Z</option><option value='-experiment__comments'>Z to A</option></select>",
                                  '<input class="search-query" type="text" name="qb_experiment_comments" placeholder="Type Experiment Comments"/>',
                                  'checkbox_qb_experiment')]
        query_builder_fields_list = list(chain(experiment_fields_list, query_builder_fields_list))
    if 'Treatment' in selected_options:
        treatments_fields_list = [('treatment_id',
                                   "<select name='qb_treatment_id_choice'><option value='' >None</option><option value='ascending'>Ascending</option><option value='descending'>Descending</option></select>",
                                   '<input class="search-query" type="text" name="qb_treatment_id" placeholder="Type a Treatment ID"/>',
                                   'checkbox_qb_treatment'), ('treatment_type',
                                                              "<select name='qb_treatment_type_choice'><option value='' >None</option><option value='alphabetical'>A to Z</option><option value='revalphabetical'>Z to A</option></select>",
                                                              '<input class="search-query" type="text" name="qb_treatment_type" placeholder="Type a Treatment Type"/>',
                                                              'checkbox_qb_treatment'), ('treatment_date',
                                                                                         "<select name='qb_treatment_date_choice'><option value='' >None</option><option value='ascending'>Ascending</option><option value='descending'>Descending</option></select>",
                                                                                         '<input class="search-query" type="text" name="qb_treatment_date" placeholder="Type Treatment Date"/>',
                                                                                         'checkbox_qb_treatment'), (
                                  'treatment_comments',
                                  "<select name='qb_treatment_comments_choice'><option value='' >None</option><option value='alphabetical'>A to Z</option><option value='revalphabetical'>Z to A</option></select>",
                                  '<input class="search-query" type="text" name="qb_treatment_comments" placeholder="Type Treatment Comments"/>',
                                  'checkbox_qb_treatment')]
        query_builder_fields_list = list(chain(treatments_fields_list, query_builder_fields_list))
    if 'ObsPlot' in selected_options:
        plot_fields_list = [('plot_id',
                             "<select name='qb_plot_id_choice'><option value='' >None</option><option value='obs_plot__plot_id'>Ascending</option><option value='-obs_plot__plot_id'>Descending</option></select>",
                             '<input class="search-query" type="text" name="qb_plot_id" placeholder="Type a Plot ID"/>',
                             'checkbox_qb_plot'), ('plot_name',
                                                   "<select name='qb_plot_name_choice'><option value='' >None</option><option value='obs_plot__plot_name'>A to Z</option><option value='-obs_plot__plot_name'>Z to A</option></select>",
                                                   '<input class="search-query" type="text" name="qb_plot_name" placeholder="Type a Plot Name"/>',
                                                   'checkbox_qb_plot'), ('plot_field_name',
                                                                         "<select name='qb_plot_field_name_choice'><option value='' >None</option><option value='field__field_name'>A to Z</option><option value='-field__field_name'>Z to A</option></select>",
                                                                         '<input class="search-query" type="text" name="qb_plot_field_name" placeholder="Type a Field Name"/>',
                                                                         'checkbox_qb_plot'), (
                            'plot_field_locality_city',
                            "<select name='qb_plot_field_locality_city_choice'><option value='' >None</option><option value='field__locality__city'>A to Z</option><option value='-field__locality__city'>Z to A</option></select>",
                            '<input class="search-query" type="text" name="qb_plot_field_locality_city" placeholder="Type a Locality City"/>',
                            'checkbox_qb_plot'), ('plot_range_num',
                                                  "<select name='qb_plot_range_choice'><option value='' >None</option><option value='obs_plot__range_num'>Ascending</option><option value='-obs_plot__range_num'>Descending</option></select>",
                                                  '<input class="search-query" type="text" name="qb_plot_range" placeholder="Type a Range Num"/>',
                                                  'checkbox_qb_plot'), ('plot_plot',
                                                                        "<select name='qb_plot_plot_choice'><option value='' >None</option><option value='obs_plot__plot'>A to Z</option><option value='-obs_plot__plot'>Z to A</option></select>",
                                                                        '<input class="search-query" type="text" name="qb_plot_plot" placeholder="Type a Plot"/>',
                                                                        'checkbox_qb_plot'), ('plot_block',
                                                                                              "<select name='qb_plot_block_choice'><option value='' >None</option><option value='obs_plot__block'>A to Z</option><option value='-obs_plot__block'>Z to A</option></select>",
                                                                                              '<input class="search-query" type="text" name="qb_plot_block" placeholder="Type a Block"/>',
                                                                                              'checkbox_qb_plot'), (
                            'plot_rep',
                            "<select name='qb_plot_rep_choice'><option value='' >None</option><option value='obs_plot__rep'>A to Z</option><option value='-obs_plot__rep'>Z to A</option></select>",
                            '<input class="search-query" type="text" name="qb_plot_rep" placeholder="Type a Rep"/>',
                            'checkbox_qb_plot'), ('plot_kernel_num',
                                                  "<select name='qb_plot_kernel_num_choice'><option value='' >None</option><option value='obs_plot__kernel_num'>Ascending</option><option value='-obs_plot__kernel_num'>Descending</option></select>",
                                                  '<input class="search-query" type="text" name="qb_plot_kernel_num" placeholder="Type Kernel Num"/>',
                                                  'checkbox_qb_plot'), ('plot_planting_date',
                                                                        "<select name='qb_plot_planting_date_choice'><option value='' >None</option><option value='obs_plot__planting_date'>Ascending</option><option value='-obs_plot__planting_date'>Descending</option></select>",
                                                                        '<input class="search-query" type="text" name="qb_plot_planting_date" placeholder="Type a Planting Date"/>',
                                                                        'checkbox_qb_plot'), ('plot_harvest_date',
                                                                                              "<select name='qb_plot_harvest_date_choice'><option value='' >None</option><option value='obs_plot__harvest_date'>Ascending</option><option value='-obs_plot__harvest_date'>Descending</option></select>",
                                                                                              '<input class="search-query" type="text" name="qb_plot_harvest_date" placeholder="Type a Harvest Date"/>',
                                                                                              'checkbox_qb_plot'), (
                            'plot_comments',
                            "<select name='qb_plot_comments_choice'><option value='' >None</option><option value='obs_plot__comments'>A to Z</option><option value='-obs_plot__comments'>Z to A</option></select>",
                            '<input class="search-query" type="text" name="qb_plot_comments" placeholder="Type a Plot Comment"/>',
                            'checkbox_qb_plot')]
        query_builder_fields_list = list(chain(plot_fields_list, query_builder_fields_list))
    if 'ObsPlant' in selected_options:
        plant_fields_list = [('plant_id',
                              "<select name='qb_plant_id_choice'><option value='' >None</option><option value='obs_plant__plant_id'>Ascending</option><option value='-obs_plant__plant_id'>Descending</option></select>",
                              '<input class="search-query" type="text" name="qb_plant_id" placeholder="Type a Plant ID"/>',
                              'checkbox_qb_plant'), ('plant_num',
                                                     "<select name='qb_plant_num_choice'><option value=''>None</option><option value='obs_plant__plant_num'>Ascending</option><option value='-obs_plant__plant_num'>Descending</option></select>",
                                                     '<input class="search-query" type="text" name="qb_plant_num" placeholder="Type a Plant Num"/>',
                                                     'checkbox_qb_plant'), ('plant_comments',
                                                                            "<select name='qb_plant_comments_choice'><option value='' >None</option><option value='obs_plant__comments'>A to Z</option><option value='-obs_plant__comments'>Z to A</option></select>",
                                                                            '<input class="search-query" type="text" name="qb_plant_comments" placeholder="Type a Plant Comment"/>',
                                                                            'checkbox_qb_plant')]
        query_builder_fields_list = list(chain(plant_fields_list, query_builder_fields_list))
    if 'ObsSample' in selected_options:
        sample_fields_list = [('sample_id',
                               "<select name='qb_sample_id_choice'><option value='' >None</option><option value='obs_sample__sample_id'>Ascending</option><option value='-obs_sample__sample_id'>Descending</option></select>",
                               '<input class="search-query" type="text" name="qb_sample_id" placeholder="Type a Sample ID"/>',
                               'checkbox_qb_sample'), ('sample_type',
                                                       "<select name='qb_sample_type_choice'><option value=''>None</option><option value='obs_sample__sample_type'>A to Z</option><option value='-obs_sample__sample_type'>Z to A</option></select>",
                                                       '<input class="search-query" type="text" name="qb_sample_type" placeholder="Type a Sample Type"/>',
                                                       'checkbox_qb_sample'), ('weight',
                                                                               "<select name='qb_sample_weight_choice'><option value='' >None</option><option value='obs_sample__weight'>Ascending</option><option value='-obs_sample__weight'>Descending</option></select>",
                                                                               '<input class="search-query" type="text" name="qb_sample_weight" placeholder="Type a Weight"/>',
                                                                               'checkbox_qb_sample'), ('kernel_num',
                                                                                                       "<select name='qb_sample_kernel_num_choice'><option value='' >None</option><option value='obs_sample__kernel_num'>Ascending</option><option value='-obs_sample__kernel_num'>Descending</option></select>",
                                                                                                       '<input class="search-query" type="text" name="qb_sample_kernel_num" placeholder="Type Kernel Num"/>',
                                                                                                       'checkbox_qb_sample'),
                              ('sample_comments',
                               "<select name='qb_sample_comments_choice'><option value='' >None</option><option value='obs_sample__comments'>A to Z</option><option value='-obs_sample__comments'>Z to A</option></select>",
                               '<input class="search-query" type="text" name="qb_sample_comments" placeholder="Type a Sample Comment"/>',
                               'checkbox_qb_sample')]
        query_builder_fields_list = list(chain(sample_fields_list, query_builder_fields_list))
    if 'ObsEnv' in selected_options:
        env_fields_list = [('environment_id',
                            "<select name='qb_env_id_choice'><option value='' >None</option><option value='obs_env__environment_id'>Ascending</option><option value='-obs_env__environment_id'>Descending</option></select>",
                            '<input class="search-query" type="text" name="qb_env_id" placeholder="Type an Environment ID"/>',
                            'checkbox_qb_env'), ('longitude',
                                                 "<select name='qb_env_longitude_choice'><option value=''>None</option><option value='obs_env__longitude'>Ascending</option><option value='-obs_env__longitude'>Descending</option></select>",
                                                 '<input class="search-query" type="text" name="qb_env_longitude" placeholder="Type a Longitude"/>',
                                                 'checkbox_qb_env'), ('latitude',
                                                                      "<select name='qb_env_latitude_choice'><option value='' >None</option><option value='obs_env__latitude'>Ascending</option><option value='-obs_env__latitude'>Descending</option></select>",
                                                                      '<input class="search-query" type="text" name="qb_env_latitude" placeholder="Type a Latitude"/>',
                                                                      'checkbox_qb_env'), ('environment_comments',
                                                                                           "<select name='qb_env_comments_choice'><option value='' >None</option><option value='obs_env__comments'>A to Z</option><option value='-obs_env__comments'>Z to A</option></select>",
                                                                                           '<input class="search-query" type="text" name="qb_env_comments" placeholder="Type Environment Comments"/>',
                                                                                           'checkbox_qb_env')]
        query_builder_fields_list = list(chain(env_fields_list, query_builder_fields_list))
    if 'ObsTissue' in selected_options:
        tissue_fields_list = [('tissue_id',
                               "<select name='qb_tissue_id_choice'><option value='' >None</option><option value='obs_tissue__tissue_id'>Ascending</option><option value='-obs_tissue__tissue_id'>Descending</option></select>",
                               '<input class="search-query" type="text" name="qb_tissue_id" placeholder="Type a Tissue ID"/>',
                               'checkbox_qb_tissue'), ('tissue_type',
                                                       "<select name='qb_tissue_type_choice'><option value=''>None</option><option value='obs_tissue__tissue_type'>A to Z</option><option value='-obs_tissue__tissue_type'>Z to A</option></select>",
                                                       '<input class="search-query" type="text" name="qb_tissue_type" placeholder="Type a Tissue Type"/>',
                                                       'checkbox_qb_tissue'), ('tissue_name',
                                                                               "<select name='qb_tissue_name_choice'><option value='' >None</option><option value='obs_tissue__tissue_name'>A to Z</option><option value='-obs_tissue__tissue_name'>Z to A</option></select>",
                                                                               '<input class="search-query" type="text" name="qb_tissue_name" placeholder="Type a Tissue Name"/>',
                                                                               'checkbox_qb_tissue'), (
                              'tissue_date_ground',
                              "<select name='qb_tissue_date_ground_choice'><option value='' >None</option><option value='obs_tissue__date_ground'>Ascending</option><option value='-obs_tissue__date_ground'>Descending</option></select>",
                              '<input class="search-query" type="text" name="qb_tissue_date_ground" placeholder="Type Date Ground"/>',
                              'checkbox_qb_tissue'), ('tissue_comments',
                                                      "<select name='qb_tissue_comments_choice'><option value='' >None</option><option value='obs_tissue__comments'>A to Z</option><option value='-obs_tissue__comments'>Z to A</option></select>",
                                                      '<input class="search-query" type="text" name="qb_tissue_comments" placeholder="Type a Tissue Comment"/>',
                                                      'checkbox_qb_tissue')]
        query_builder_fields_list = list(chain(tissue_fields_list, query_builder_fields_list))
    if 'ObsPlate' in selected_options:
        plate_fields_list = [('plate_id',
                              "<select name='qb_plate_id_choice'><option value='' >None</option><option value='obs_plate__plate_id'>Ascending</option><option value='-obs_plate__plate_id'>Descending</option></select>",
                              '<input class="search-query" type="text" name="qb_plate_id" placeholder="Type a Plate ID"/>',
                              'checkbox_qb_plate'), ('plate_name',
                                                     "<select name='qb_plate_name_choice'><option value=''>None</option><option value='obs_plate__plate_name'>A to Z</option><option value='-obs_plate__plate_name'>Z to A</option></select>",
                                                     '<input class="search-query" type="text" name="qb_plate_name" placeholder="Type a Plate Name"/>',
                                                     'checkbox_qb_plate'), ('plate_date',
                                                                            "<select name='qb_plate_date_choice'><option value='' >None</option><option value='obs_plate__date'>Ascending</option><option value='-obs_plate__date'>Descending</option></select>",
                                                                            '<input class="search-query" type="text" name="qb_plate_date" placeholder="Type a Plate Date"/>',
                                                                            'checkbox_qb_plate'), ('plate_contents',
                                                                                                   "<select name='qb_plate_contents_choice'><option value='' >None</option><option value='obs_plate__contents'>Ascending</option><option value='-obs_plate__contents'>Descending</option></select>",
                                                                                                   '<input class="search-query" type="text" name="qb_plate_contents" placeholder="Type Plate Contents"/>',
                                                                                                   'checkbox_qb_plate'),
                             ('plate_rep',
                              "<select name='qb_plate_rep_choice'><option value='' >None</option><option value='obs_plate__rep'>Ascending</option><option value='-obs_plate__rep'>Descending</option></select>",
                              '<input class="search-query" type="text" name="qb_plate_rep" placeholder="Type Plate Rep"/>',
                              'checkbox_qb_plate'), ('plate_type',
                                                     "<select name='qb_plate_type_choice'><option value='' >None</option><option value='obs_plate__plate_type'>A to Z</option><option value='-obs_plate__plate_type'>Z to A</option></select>",
                                                     '<input class="search-query" type="text" name="qb_plate_type" placeholder="Type a Plate Type"/>',
                                                     'checkbox_qb_plate'), ('plate_status',
                                                                            "<select name='qb_plate_status_choice'><option value='' >None</option><option value='obs_plate__plate_status'>A to Z</option><option value='-obs_plate__plate_status'>Z to A</option></select>",
                                                                            '<input class="search-query" type="text" name="qb_plate_status" placeholder="Type a Plate Status"/>',
                                                                            'checkbox_qb_plate'), ('plate_comments',
                                                                                                   "<select name='qb_plate_comments_choice'><option value='' >None</option><option value='obs_plate__comments'>A to Z</option><option value='-obs_plate__comments'>Z to A</option></select>",
                                                                                                   '<input class="search-query" type="text" name="qb_plate_comments" placeholder="Type a Plate Comment"/>',
                                                                                                   'checkbox_qb_plate')]
        query_builder_fields_list = list(chain(plate_fields_list, query_builder_fields_list))
    if 'ObsWell' in selected_options:
        well_fields_list = [('well_id',
                             "<select name='qb_well_id_choice'><option value='' >None</option><option value='obs_well__well_id'>Ascending</option><option value='-obs_well__well_id'>Descending</option></select>",
                             '<input class="search-query" type="text" name="qb_well_id" placeholder="Type a Well ID"/>',
                             'checkbox_qb_well'), ('well_well',
                                                   "<select name='qb_well_well_choice'><option value=''>None</option><option value='obs_well__well'>A to Z</option><option value='-obs_well__well'>Z to A</option></select>",
                                                   '<input class="search-query" type="text" name="qb_well_well" placeholder="Type a Well"/>',
                                                   'checkbox_qb_well'), ('well_inventory',
                                                                         "<select name='qb_well_inventory_choice'><option value='' >None</option><option value='obs_well__well_inventory'>A to Z</option><option value='-obs_well__well_inventory'>Z to A</option></select>",
                                                                         '<input class="search-query" type="text" name="qb_well_inventory" placeholder="Type a Well Inventory"/>',
                                                                         'checkbox_qb_well'), ('well_tube_label',
                                                                                               "<select name='qb_well_tube_label_choice'><option value='' >None</option><option value='obs_well__tube_label'>Ascending</option><option value='-obs_well__tube_label'>Descending</option></select>",
                                                                                               '<input class="search-query" type="text" name="qb_well_tube_label" placeholder="Type Tube Label"/>',
                                                                                               'checkbox_qb_well'), (
                            'well_comments',
                            "<select name='qb_well_comments_choice'><option value='' >None</option><option value='obs_well__comments'>A to Z</option><option value='-obs_well__comments'>Z to A</option></select>",
                            '<input class="search-query" type="text" name="qb_well_comments" placeholder="Type a Well Comment"/>',
                            'checkbox_qb_well')]
        query_builder_fields_list = list(chain(well_fields_list, query_builder_fields_list))
    if 'ObsDNA' in selected_options:
        dna_fields_list = [('dna_id',
                            "<select name='qb_dna_id_choice'><option value='' >None</option><option value='obs_dna__dna_id'>Ascending</option><option value='-obs_dna__dna_id'>Descending</option></select>",
                            '<input class="search-query" type="text" name="qb_dna_id" placeholder="Type a DNA ID"/>',
                            'checkbox_qb_dna'), ('dna_extraction_method',
                                                 "<select name='qb_dna_extraction_method_choice'><option value=''>None</option><option value='obs_dna__extraction_method'>A to Z</option><option value='-obs_dna__extraction_method'>Z to A</option></select>",
                                                 '<input class="search-query" type="text" name="qb_dna_extraction_method" placeholder="Type a DNA Extraction"/>',
                                                 'checkbox_qb_dna'), ('dna_date',
                                                                      "<select name='qb_dna_date_choice'><option value='' >None</option><option value='obs_dna__date'>A to Z</option><option value='-obs_dna__date'>Z to A</option></select>",
                                                                      '<input class="search-query" type="text" name="qb_dna_date" placeholder="Type a DNA Date"/>',
                                                                      'checkbox_qb_dna'), ('dna_tube_id',
                                                                                           "<select name='qb_dna_tube_id_choice'><option value='' >None</option><option value='obs_dna__tube_id'>Ascending</option><option value='-obs_dna__tube_id'>Descending</option></select>",
                                                                                           '<input class="search-query" type="text" name="qb_dna_tube_id" placeholder="Type DNA Tube ID"/>',
                                                                                           'checkbox_qb_dna'), (
                           'dna_tube_type',
                           "<select name='qb_dna_tube_type_choice'><option value='' >None</option><option value='obs_dna__tube_type'>Ascending</option><option value='-obs_dna__tube_type'>Descending</option></select>",
                           '<input class="search-query" type="text" name="qb_dna_tube_type" placeholder="Type DNA Tube Type"/>',
                           'checkbox_qb_dna'), ('dna_comments',
                                                "<select name='qb_dna_comments_choice'><option value='' >None</option><option value='obs_dna__comments'>A to Z</option><option value='-obs_dna__comments'>Z to A</option></select>",
                                                '<input class="search-query" type="text" name="qb_dna_comments" placeholder="Type a DNA Comment"/>',
                                                'checkbox_qb_dna')]
        query_builder_fields_list = list(chain(dna_fields_list, query_builder_fields_list))
    if 'ObsCulture' in selected_options:
        culture_fields_list = [('culture_id',
                                "<select name='qb_culture_id_choice'><option value='' >None</option><option value='obs_culture__culture_id'>Ascending</option><option value='-obs_culture__culture_id'>Descending</option></select>",
                                '<input class="search-query" type="text" name="qb_culture_id" placeholder="Type a Culture ID"/>',
                                'checkbox_qb_culture'), ('culture_name',
                                                         "<select name='qb_culture_name_choice'><option value=''>None</option><option value='obs_culture__culture_name'>A to Z</option><option value='-obs_culture__culture_name'>Z to A</option></select>",
                                                         '<input class="search-query" type="text" name="qb_culture_name" placeholder="Type a Culture Name"/>',
                                                         'checkbox_qb_culture'), ('culture_microbe_type',
                                                                                  "<select name='qb_culture_microbe_type_choice'><option value='' >None</option><option value='obs_culture__microbe_type'>A to Z</option><option value='-obs_culture__microbe_type'>Z to A</option></select>",
                                                                                  '<input class="search-query" type="text" name="qb_culture_microbe_type" placeholder="Type a Culture Microbe Type"/>',
                                                                                  'checkbox_qb_culture'), (
                               'culture_plating_cycle',
                               "<select name='qb_culture_plating_cycle_choice'><option value='' >None</option><option value='obs_culture__plating_cycle'>Ascending</option><option value='-obs_culture__plating_cycle'>Descending</option></select>",
                               '<input class="search-query" type="text" name="qb_culture_plating_cycle" placeholder="Type Culture Plating Cycle"/>',
                               'checkbox_qb_culture'), ('culture_dilution',
                                                        "<select name='qb_culture_dilution_choice'><option value='' >None</option><option value='obs_culture__dilution'>Ascending</option><option value='-obs_culture__dilution'>Descending</option></select>",
                                                        '<input class="search-query" type="text" name="qb_culture_dilution" placeholder="Type a Culture Dilution"/>',
                                                        'checkbox_qb_culture'), ('culture_image_filename',
                                                                                 "<select name='qb_culture_image_filename_choice'><option value='' >None</option><option value='obs_culture__image_filename'>Ascending</option><option value='-obs_culture__image_filename'>Descending</option></select>",
                                                                                 '<input class="search-query" type="text" name="qb_culture_image_filename" placeholder="Type a Culture Image File"/>',
                                                                                 'checkbox_qb_culture'), (
                               'culture_comments',
                               "<select name='qb_culture_comments_choice'><option value='' >None</option><option value='obs_culture__comments'>Ascending</option><option value='-obs_culture__comments'>Descending</option></select>",
                               '<input class="search-query" type="text" name="qb_culture_comments" placeholder="Type a Culture Comment"/>',
                               'checkbox_qb_culture'), ('culture_medium_name',
                                                        "<select name='qb_culture_medium_name_choice'><option value='' >None</option><option value='obs_culture__medium__media_name'>Ascending</option><option value='-obs_culture__medium__media_name'>Descending</option></select>",
                                                        '<input class="search-query" type="text" name="qb_culture_medium_name" placeholder="Type a Culture Medium Name"/>',
                                                        'checkbox_qb_culture'), ('culture_medium_type',
                                                                                 "<select name='qb_culture_medium_type_choice'><option value='' >None</option><option value='obs_culture__medium__media_type'>Ascending</option><option value='-obs_culture__medium__media_type'>Descending</option></select>",
                                                                                 '<input class="search-query" type="text" name="qb_culture_medium_type" placeholder="Type a Culture Medium Type"/>',
                                                                                 'checkbox_qb_culture'), (
                               'culture_medium_description',
                               "<select name='qb_culture_medium_description_choice'><option value='' >None</option><option value='obs_culture__medium__media_description'>Ascending</option><option value='-obs_culture__medium__media_description'>Descending</option></select>",
                               '<input class="search-query" type="text" name="qb_culture_medium_description" placeholder="Type a Culture Medium Description"/>',
                               'checkbox_qb_culture'), ('culture_medium_preparation',
                                                        "<select name='qb_culture_medium_preparation_choice'><option value='' >None</option><option value='obs_culture__medium__media_preparation'>Ascending</option><option value='-obs_culture__medium__media_preparation'>Descending</option></select>",
                                                        '<input class="search-query" type="text" name="qb_culture_medium_preparation" placeholder="Type a Culture Media Prep"/>',
                                                        'checkbox_qb_culture'), ('culture_medium_comments',
                                                                                 "<select name='qb_culture_medium_comments_choice'><option value='' >None</option><option value='obs_culture__medium__comments'>Ascending</option><option value='-obs_culture__medium__comments'>Descending</option></select>",
                                                                                 '<input class="search-query" type="text" name="qb_culture_medium_comments" placeholder="Type a Culture Medium Comment"/>',
                                                                                 'checkbox_qb_culture'), (
                               'culture_medium_citation_type',
                               "<select name='qb_culture_medium_citation_type_choice'><option value='' >None</option><option value='obs_culture__medium__citation__citation_type'>Ascending</option><option value='-obs_culture__medium__citation__citation_type'>Descending</option></select>",
                               '<input class="search-query" type="text" name="qb_culture_medium_citation_type" placeholder="Type a Culture Medium Citation Type"/>',
                               'checkbox_qb_culture'), ('culture_medium_citation_title',
                                                        "<select name='qb_culture_medium_citation_title_choice'><option value='' >None</option><option value='obs_culture__medium__citation__title'>A to Z</option><option value='-obs_culture__medium__citation__title'>Z to A</option></select>",
                                                        '<input class="search-query" type="text" name="qb_culture_medium_citation_title" placeholder="Type a Culture Medium Citation Title"/>',
                                                        'checkbox_qb_culture'), ('culture_medium_citation_url',
                                                                                 "<select name='qb_culture_medium_citation_url_choice'><option value='' >None</option><option value='obs_culture__medium__citation__url'>A to Z</option><option value='-obs_culture__medium__citation__url'>Z to A</option></select>",
                                                                                 '<input class="search-query" type="text" name="qb_culture_medium_citation_url" placeholder="Type a Culture Medium Citation URL"/>',
                                                                                 'checkbox_qb_culture'), (
                               'culture_medium_citation_pubmed',
                               "<select name='qb_culture_medium_citation_pubmed_choice'><option value='' >None</option><option value='obs_culture__medium__citation__pubmed_id'>A to Z</option><option value='-obs_culture__medium__citation__pubmed_id>Z to A</option></select>",
                               '<input class="search-query" type="text" name="qb_culture_medium_citation_pubmed" placeholder="Type a Culture Medium Citation Pubmed"/>',
                               'checkbox_qb_culture'), ('culture_medium_citation_comments',
                                                        "<select name='qb_culture_medium_citation_comments_choice'><option value='' >None</option><option value='obs_culture__medium__citation__comments'>A to Z</option><option value='-obs_culture__medium__citation__comments'>Z to A</option></select>",
                                                        '<input class="search-query" type="text" name="qb_culture_medium_citation_comments" placeholder="Type a Culture Medium Citation Comment"/>',
                                                        'checkbox_qb_culture')]
...
def query_builder_fields(request):
    ...
    if request.POST.getlist('checkbox_qb_measurement', False):
        ...
        measurement_args = [
            ('measurement_time', 'qb_measurement_time', 'qb_measurement_time_choice', 'time_of_measurement__contains'),
            ('measurement_value', 'qb_measurement_value', 'qb_measurement_value_choice', 'value__contains'),
            ('measurement_comments', 'qb_measurement_comments', 'qb_measurement_comments_choice', 'comments__contains'),
            ('measurement_parameter', 'qb_measurement_parameter', 'qb_measurement_parameter_choice',
             'measurement_parameter__parameter__contains'), (
            'measurement_parameter_type', 'qb_measurement_type', 'qb_measurement_type_choice',
            'measurement_parameter__parameter_type__contains'), (
            'measurement_protocol', 'qb_measurement_protocol', 'qb_measurement_protocol_choice',
            'measurement_parameter__protocol__contains'), (
            'measurement_unit_of_measure', 'qb_measurement_unit', 'qb_measurement_unit_choice',
            'measurement_parameter__unit_of_measure__contains'), (
    ...
    if request.POST.getlist('checkbox_qb_experiment', False):
        ...
        experiment_args = [
            ('experiment_name', 'qb_experiment_name', 'qb_experiment_name_choice', 'experiment__name__contains'), (
            'experiment_field_name', 'qb_experiment_field_name', 'qb_experiment_field_name_choice',
            'experiment__field__field_name__contains'), (
            'experiment_field_locality_city', 'qb_experiment_field_locality_city',
            'qb_experiment_field_locality_city_choice', 'experiment__field__locality__city__contains'), (
            'experiment_start_date', 'qb_experiment_date', 'qb_experiment_date_choice',
            'experiment__start_date__contains'), (
            'experiment_purpose', 'qb_experiment_purpose', 'qb_experiment_purpose_choice',
            'experiment__purpose__contains'), (
            'experiment_comments', 'qb_experiment_comments', 'qb_experiment_comments_choice',
            'experiment__comments__contains')]
    ...
    if request.POST.getlist('checkbox_qb_plot', False):
        obs_tracker_fields = list(chain(obs_tracker_fields, request.POST.getlist('checkbox_qb_plot')))
        display_fields = list(chain(display_fields, request.POST.getlist('checkbox_qb_plot')))
        plot_args = [('plot_id', 'qb_plot_id', 'qb_plot_id_choice', 'obs_plot__plot_id__contains'),
                     ('plot_name', 'qb_plot_name', 'qb_plot_name_choice', 'obs_plot__plot_name__contains'), (
                     'plot_field_name', 'qb_plot_field_name', 'qb_plot_field_name_choice',
                     'field__field_name__contains'), (
                     'plot_field_locality_city', 'qb_plot_field_locality_city', 'qb_plot_field_locality_city_choice',
                     'field__locality__city__contains'),
                     ('plot_range_num', 'qb_plot_range', 'qb_plot_range_choice', 'obs_plot__range_num__contains'),
                     ('plot_plot', 'qb_plot_plot', 'qb_plot_plot_choice', 'obs_plot__plot__contains'),
                     ('plot_block', 'qb_plot_block', 'qb_plot_block_choice', 'obs_plot__block__contains'),
                     ('plot_rep', 'qb_plot_rep', 'qb_plot_rep_choice', 'obs_plot__rep__contains'), (
                     'plot_kernel_num', 'qb_plot_kernel_num', 'qb_plot_kernel_num_choice',
                     'obs_plot__kernel_num__contains'), (
                     'plot_planting_date', 'qb_plot_planting_date', 'qb_plot_planting_date_choice',
                     'obs_plot__planting_date__contains'), (
                     'plot_harvest_date', 'qb_plot_harvest_date', 'qb_plot_harvest_date_choice',
                     'obs_plot__harvest_date__contains'),
                     ('plot_comments', 'qb_plot_comments', 'qb_plot_comments_choice', 'obs_plot__comments__contains')]
        obs_tracker_args.extend(plot_args)
    ...
    if request.POST.getlist('checkbox_qb_plant', False):
        ...
        plant_args = [('plant_id', 'qb_plant_id', 'qb_plant_id_choice', 'obs_plant__plant_id__contains'),
                      ('plant_num', 'qb_plant_num', 'qb_plant_num_choice', 'obs_plant__plant_num__contains'), (
                      'plant_comments', 'qb_plant_comments', 'qb_plant_comments_choice',
                      'obs_plant__comments__contains')]
    ...
    if request.POST.getlist('checkbox_qb_tissue', False):
        ...
        tissue_args = [('tissue_id', 'qb_tissue_id', 'qb_tissue_id_choice', 'obs_tissue__tissue_id__contains'),
                       ('tissue_type', 'qb_tissue_type', 'qb_tissue_type_choice', 'obs_tissue__tissue_type__contains'),
                       ('tissue_name', 'qb_tissue_name', 'qb_tissue_name_choice', 'obs_tissue__tissue_name__contains'),
                       ('tissue_date_ground', 'qb_tissue_date_ground', 'qb_tissue_date_ground_choice',
                        'obs_tissue__date_ground__contains'), (
                       'tissue_comments', 'qb_tissue_comments', 'qb_tissue_comments_choice',
                       'obs_tissue__comments__contains')]
    ...
    if request.POST.getlist('checkbox_qb_plate', False):
        ...
        plate_args = [('plate_id', 'qb_plate_id', 'qb_plate_id_choice', 'obs_plate__plate_id__contains'),
                      ('plate_name', 'qb_plate_name', 'qb_plate_name_choice', 'obs_plate__plate_name__contains'),
                      ('plate_date', 'qb_plate_date', 'qb_plate_date_choice', 'obs_plate__date__contains'), (
                      'plate_contents', 'qb_plate_contents', 'qb_plate_contents_choice',
                      'obs_plate__contents__contains'),
                      ('plate_rep', 'qb_plate_rep', 'qb_plate_rep_choice', 'obs_plate__rep__contains'),
                      ('plate_type', 'qb_plate_type', 'qb_plate_type_choice', 'obs_plate__plate_type__contains'), (
                      'plate_status', 'qb_plate_status', 'qb_plate_status_choice', 'obs_plate__plate_status__contains'),
                      ('plate_comments', 'qb_plate_comments', 'qb_plate_comments_choice',
                       'obs_plate__comments__contains')]
    ...
    if request.POST.getlist('checkbox_qb_well', False):
        ...
        well_args = [('well_id', 'qb_well_id', 'qb_well_id_choice', 'obs_well__well_id__contains'),
                     ('well_well', 'qb_well_well', 'qb_well_well_choice', 'obs_well__well__contains'), (
                     'well_inventory', 'qb_well_inventory', 'qb_well_inventory_choice',
                     'obs_well__well_inventory__contains'), (
                     'well_tube_label', 'qb_well_tube_label', 'qb_well_tube_label_choice',
                     'obs_well__tube_label__contains'),
                     ('well_comments', 'qb_well_comments', 'qb_well_comments_choice', 'obs_well__comments__contains')]
    ...
    if request.POST.getlist('checkbox_qb_culture', False):
        ...
        culture_args = [('culture_id', 'qb_culture_id', 'qb_culture_id_choice', 'obs_culture__culture_id__contains'), (
        'culture_name', 'qb_culture_name', 'qb_culture_name_choice', 'obs_culture__culture_name__contains'), (
                        'culture_microbe_type', 'qb_culture_microbe_type', 'qb_culture_microbe_type_choice',
                        'obs_culture__microbe_type__contains'), (
                        'culture_plating_cycle', 'qb_culture_plating_cycle', 'qb_culture_plating_cycle_choice',
                        'obs_well__plating_cycle__contains'), (
                        'culture_dilution', 'qb_culture_dilution', 'qb_culture_dilution_choice',
                        'obs_culture__dilution__contains'), (
                        'culture_image_filename', 'qb_culture_image_filename', 'qb_culture_image_filename_choice',
                        'obs_culture__image_filename__contains'), (
                        'culture_comments', 'qb_culture_comments', 'qb_culture_comments_choice',
                        'obs_culture__comments__contains'), (
                        'culture_medium_name', 'qb_culture_medium_name', 'qb_culture_medium_name_choice',
                        'obs_culture__medium__media_name__contains'), (
                        'culture_medium_type', 'qb_culture_medium_type', 'qb_culture_medium_type_choice',
                        'obs_culture__medium__media_type__contains'), (
                        'culture_medium_description', 'qb_culture_medium_description',
                        'qb_culture_medium_description_choice', 'obs_culture__medium__media_description__contains'), (
                        'culture_medium_preparation', 'qb_culture_medium_preparation',
                        'qb_culture_medium_preparation_choice', 'obs_culture__medium__media_preparation__contains'), (
                        'culture_medium_comments', 'qb_culture_medium_comments', 'qb_culture_medium_comments_choice',
                        'obs_culture__medium__comments__contains'), (
                        'culture_medium_citation_type', 'qb_culture_medium_citation_type',
                        'qb_culture_medium_citation_type_choice',
                        'obs_culture__medium__citation__citation_type__contains'), (
                        'culture_medium_citation_title', 'qb_culture_medium_citation_title',
                        'qb_culture_medium_citation_title', 'obs_culture__medium__citation__title__contains'), (
                        'culture_medium_citation_url', 'qb_culture_medium_citation_url',
                        'qb_culture_medium_citation_url_choice', 'obs_culture__medium__citation__title__contains'), (
                        'culture_medium_citation_url', 'qb_culture_medium_citation_url',
                        'qb_culture_medium_citation_url_choice', 'obs_culture__medium__citation__url__contains'), (
                        'culture_medium_citation_pubmed', 'qb_culture_medium_citation_pubmed',
                        'qb_culture_medium_citation_pubmed_choice',
                        'obs_culture__medium__citation__pubmed_id__contains'), (
                        'culture_medium_citation_comments', 'qb_culture_medium_citation_comments',
                        'qb_culture_medium_citation_comments_choice',
                        'obs_culture__medium__citation__comments__contains')]
    ...
    if request.POST.getlist('checkbox_qb_dna', False):
        ...
        dna_args = [('dna_id', 'qb_dna_id', 'qb_dna_id_choice', 'obs_dna__dna_id__contains'), (
        'dna_extraction_method', 'qb_dna_extraction_method', 'qb_dna_extraction_method_choice',
        'obs_dna__extraction_method__contains'),
                    ('dna_date', 'qb_dna_date', 'qb_dna_date_choice', 'obs_dna__date__contains'),
                    ('dna_tube_id', 'qb_dna_tube_id', 'qb_dna_tube_id_choice', 'obs_dna__tube_id__contains'),
                    ('dna_tube_type', 'qb_dna_tube_type', 'qb_dna_tube_type_choice', 'obs_dna__tube_type__contains'),
                    ('dna_comments', 'qb_dna_comments_type', 'qb_dna_comments_choice', 'obs_dna__comments__contains')]
    ...
    if request.POST.getlist('checkbox_qb_stock', False):
        ...
        stock_args = [('stock_seed_id', 'qb_stock_seed_id', 'qb_stock_seed_id_choice', 'stock__seed_id__contains'), (
        'stock_seed_name', 'qb_stock_seed_name', 'qb_stock_seed_name_choice', 'stock__seed_name__contains'),
                      ('stock_cross_type', 'qb_stock_cross', 'qb_stock_cross_choice', 'stock__cross_type__contains'),
                      ('stock_pedigree', 'qb_stock_pedigree', 'qb_stock_pedigree_choice', 'stock__pedigree__contains'),
                      ('stock_status', 'qb_stock_status', 'qb_stock_status_choice', 'stock__stock_status__contains'),
                      ('stock_date', 'qb_stock_date', 'qb_stock_date_choice', 'stock__stock_date__contains'),
                      ('stock_inoculated', 'qb_stock_inoculated', '', 'stock__inoculated'),
                      ('stock_comments', 'qb_stock_comments', 'qb_stock_comments_choice', 'stock__comments__contains'),
                      ('stock_binomial', 'qb_stock_binomial', 'qb_stock_binomial_choice',
                       'stock__passport__taxonomy__binomial__contains'), (
                      'stock_population', 'qb_stock_population', 'qb_stock_population_choice',
                      'stock__passport__taxonomy__population__contains'), (
                      'stock_collection_date', 'qb_stock_collecting_date', 'qb_stock_collecting_date_choice',
                      'stock__passport__collecting__collection_date__contains'), (
                      'stock_collection_method', 'qb_collecting_method', 'qb_collecting_method_choice',
                      'stock__passport__collecting__collection_method__contains'), (
                      'stock_collection_comments', 'qb_stock_collecting_comments',
                      'qb_stock_collecting_comments_choice', 'stock__passport__collecting__comments__contains'), (
                      'stock_source_organization', 'qb_stock_people_org', 'qb_stock_people_org_choice',
                      'stock__passport__people__organization__contains')]
        obs_tracker_args.extend(stock_args)

    model_field_mapper = {'measurement_time': 'time_of_measurement', 'measurement_value': 'value',
                          'measurement_comments': 'comments',
                          'measurement_parameter': 'measurement_parameter__parameter',
                          'measurement_parameter_type': 'measurement_parameter__parameter_type',
                          'measurement_protocol': 'measurement_parameter__protocol',
                          'measurement_unit_of_measure': 'measurement_parameter__unit_of_measure',
                          'measurement_trait_id_buckler': 'measurement_parameter__trait_id_buckler',
                          'experiment_name': 'experiment__name',
                          'experiment_field_name': 'experiment__field__field_name',
                          'experiment_field_locality_city': 'experiment__field__locality__city',
                          'experiment_start_date': 'experiment__start_date',
                          'experiment_purpose': 'experiment__purpose', 'experiment_comments': 'experiment__comments',
                          'plot_id': 'obs_plot__plot_id', 'plot_name': 'obs_plot__plot_name',
                          'plot_field_name': 'field__field_name', 'plot_field_locality_city': 'field__locality__city',
                          'plot_range_num': 'obs_plot__range_num', 'plot_plot': 'obs_plot__plot',
                          'plot_block': 'obs_plot__block', 'plot_rep': 'obs_plot__rep',
                          'plot_kernel_num': 'obs_plot__kernel_num', 'plot_planting_date': 'obs_plot__planting_date',
                          'plot_harvest_date': 'obs_plot__harvest_date', 'plot_comments': 'obs_plot__comments',
                          'plant_id': 'obs_plant__plant_id', 'plant_num': 'obs_plant__plant_num',
                          'plant_comments': 'obs_plant__comments', 'stock_seed_id': 'stock__seed_id',
                          'stock_seed_name': 'stock__seed_name', 'stock_cross_type': 'stock__cross_type',
                          'stock_pedigree': 'stock__pedigree', 'stock_status': 'stock__stock_status',
                          'stock_date': 'stock__stock_date', 'stock_inoculated': 'stock__inoculated',
                          'stock_comments': 'stock__comments', 'stock_binomial': 'stock__passport__taxonomy__binomial',
                          'stock_population': 'stock__passport__taxonomy__population',
                          'stock_collection_date': 'stock__passport__collecting__collection_date',
                          'stock_collection_method': 'stock__passport__collecting__collection_method',
                          'stock_collection_comments': 'stock__passport__collecting__comments',
                          'stock_source_organization': 'stock__passport__people__organization',
                          'tissue_id': 'obs_tissue__tissue_id', 'tissue_type': 'obs_tissue__tissue_type',
                          'tissue_name': 'obs_tissue__tissue_name', 'tissue_date_ground': 'obs_tissue__date_ground',
                          'tissue_comments': 'obs_tissue__comments', 'plate_id': 'obs_plate__plate_id',
                          'plate_name': 'obs_plate__plate_name', 'plate_date': 'obs_plate__date',
                          'plate_contents': 'obs_plate__contents', 'plate_rep': 'obs_plate__rep',
                          'plate_type': 'obs_plate__plate_type', 'plate_status': 'obs_plate__plate_status',
                          'plate_comments': 'obs_plate__comments', 'well_id': 'obs_well__well_id',
                          'well_well': 'obs_well__well', 'well_inventory': 'obs_well__well_inventory',
                          'well_tube_label': 'obs_well__tube_label', 'well_comments': 'obs_well__comments',
                          'culture_id': 'obs_culture__culture_id', 'culture_name': 'obs_culture__culture_name',
                          'culture_microbe_type': 'obs_culture__microbe_type',
                          'culture_plating_cycle': 'obs_culture__plating_cycle',
                          'culture_dilution': 'obs_culture__dilution',
                          'culture_image_filename': 'obs_culture__image_filename',
                          'culture_comments': 'obs_culture__comments',
                          'culture_medium_type': 'obs_culture__medium__media_type',
                          'culture_medium_name': 'obs_culture__medium__media_name',
                          'culture_medium_description': 'obs_culture__medium__media_description',
                          'culture_medium_preparation': 'obs_culture__medium__media_preparation',
                          'culture_medium_comments': 'obs_culture__medium__comments',
                          'culture_medium_citation_type': 'obs_culture__medium__citation__citation_type',
                          'culture_medium_citation_title': 'obs_culture__medium__citation__title',
                          'culture_medium_citation_url': 'obs_culture__medium__citation__url',
                          'culture_medium_citation_pubmed': 'obs_culture__medium__citation__pubmed_id',
                          'culture_medium_citation_comments': 'obs_culture__medium__citation__comments',
                          'dna_id': 'obs_dna__dna_id', 'dna_extraction_method': 'obs_dna__extraction_method',
                          'dna_date': 'obs_dna__date', 'dna_tube_id': 'obs_dna__tube_id',
                          'dna_tube_type': 'obs_dna__tube_type', 'dna_comments': 'obs_dna__comments'}
    ...
    try:
        obs_tracker_ordered_results = ObsTracker.objects.filter(**obs_tracker_kwargs).values(
            *obs_tracker_model_fields).order_by(*obs_tracker_order)
    except ObsTracker.DoesNotExist:
